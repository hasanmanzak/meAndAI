...
function Invoke-MeAndAIQuickAdoption {
    param([string]$TargetPath = '.', [string]$ProtocolTag = 'v0.15.4')
...
if (Test-Path -LiteralPath $builderPath -PathType Leaf) {
    ...
    try {
        ...
        [void](& $builderPath -SourceRoot $sourceRoot `
            -RuntimeReleaseTag 'v0.15.4' -SourceCommit $sourceCommit `
            -OutputPath $first)
        [void](& $builderPath -SourceRoot $sourceRoot `
            -RuntimeReleaseTag 'v0.15.4' -SourceCommit $sourceCommit `
            -OutputPath $second)
        if ($PSVersionTable.PSEdition -ceq 'Desktop') {
            ...
            [void](& $fixtureBuilderPath -RuntimeReleaseTag 'v0.15.4' `
        ...
        try {
            ...
            if ([string]$manifest.sourceCommit -cne $sourceCommit -or
                [string]$manifest.runtimeReleaseTag -cne 'v0.15.4') {
...
"@ -replace "`n", "`r`n"
        ...
        foreach ($scenario in @(
            ...
            if ($scenario.ShouldPass) {
                $expectedSentinel = "$runtimeRoot`nv0.15.4"
