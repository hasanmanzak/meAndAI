...
function Invoke-MeAndAIQuickAdoption {
    param([string]$TargetPath = '.', [string]$ProtocolTag = 'v0.14.1')
...
if (Test-Path -LiteralPath $builderPath -PathType Leaf) {
    ...
    try {
        ...
        [void](& $builderPath -SourceRoot $sourceRoot `
            -RuntimeReleaseTag 'v0.14.1' -SourceCommit $sourceCommit `
            -OutputPath $first)
        [void](& $builderPath -SourceRoot $sourceRoot `
            -RuntimeReleaseTag 'v0.14.1' -SourceCommit $sourceCommit `
            -OutputPath $second)
        if ($PSVersionTable.PSEdition -ceq 'Desktop') {
            ...
            [void](& $fixtureBuilderPath -RuntimeReleaseTag 'v0.14.1' `
        ...
        try {
            ...
            if ([string]$manifest.sourceCommit -cne $sourceCommit -or
                [string]$manifest.runtimeReleaseTag -cne 'v0.14.1') {
...
"@ -replace "`n", "`r`n"
        ...
        foreach ($scenario in @(
            ...
            if ($scenario.ShouldPass) {
                $expectedSentinel = "$runtimeRoot`nv0.14.1"
