...
function Invoke-MeAndAIQuickAdoption {
    param([string]$TargetPath = '.', [string]$ProtocolTag = 'v0.12.6')
...
if (Test-Path -LiteralPath $builderPath -PathType Leaf) {
    ...
    try {
        ...
        [void](& $builderPath -SourceRoot $sourceRoot `
            -RuntimeReleaseTag 'v0.12.6' -SourceCommit $sourceCommit `
            -OutputPath $first)
        [void](& $builderPath -SourceRoot $sourceRoot `
            -RuntimeReleaseTag 'v0.12.6' -SourceCommit $sourceCommit `
            -OutputPath $second)
        if ($PSVersionTable.PSEdition -ceq 'Desktop') {
            ...
            [void](& $fixtureBuilderPath -RuntimeReleaseTag 'v0.12.6' `
        ...
        try {
            ...
            if ([string]$manifest.sourceCommit -cne $sourceCommit -or
                [string]$manifest.runtimeReleaseTag -cne 'v0.12.6') {
...
"@ -replace "`n", "`r`n"
        ...
        foreach ($scenario in @(
            ...
            if ($scenario.ShouldPass) {
                $expectedSentinel = "$runtimeRoot`nv0.12.6"
