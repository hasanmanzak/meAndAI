...
$script:QuickAdoptionSuiteOwnerPath = [IO.Path]::GetFullPath($PSCommandPath)
$script:QuickAdoptionSuiteOwnerDigest = 'sha256:' +
    ([string](Get-FileHash -LiteralPath $script:QuickAdoptionSuiteOwnerPath `
...
$script:QuickAdoptionTestRepositoryModule = @(
    Import-Module $testRepositoryPath -Force -PassThru
)[0]
if ($null -eq $script:QuickAdoptionTestRepositoryModule.ExportedCommands[
        'Invoke-MeAndAITestRepositoryGit'
    ]) {
    throw 'Quick-adoption could not retain the canonical test repository command.'
}
...
$quickAllOperationExpectation = Resolve-MeAndAITestOperationExpectation `
    -Contract $operationContract `
    -Owner 'tests/capabilities/initial-adoption/quick-adoption.tests.ps1' `
    -SuiteArguments @()
$operationExpectation = if ($Shard -ceq 'All') {
    $quickAllOperationExpectation
}
else { $null }

function Get-QuickAdoptionOperationMaximum {
    param([Parameter(Mandatory)][string]$Name)

    $matches = @($quickAllOperationExpectation.Counters | Where-Object {
        [string]$_.Name -ceq $Name
    })
    if ($matches.Count -ne 1) {
        throw "Quick-adoption operation counter '$Name' is not declared exactly once."
    }
    return [long]$matches[0].Maximum
}

$credentialContainmentGitProcessMaximum =
    Get-QuickAdoptionOperationMaximum `
        -Name 'credential-containment.git-process-per-scan'
$githubCliVersionFullLauncherMaximum =
    Get-QuickAdoptionOperationMaximum `
...
$contractAdapterPath = $launcherPath
$launcherSourcePaths = @(
...
$script:QuickAdoptionReusableFixtures =
    [System.Collections.Generic.Dictionary[string, object]]::new(
        [StringComparer]::Ordinal
    )
$script:QuickAdoptionReusableBuilds =
    [System.Collections.Generic.Dictionary[string, int]]::new(
        [StringComparer]::Ordinal
    )
$script:QuickAdoptionReusableRequests =
    [System.Collections.Generic.Dictionary[string, int]]::new(
        [StringComparer]::Ordinal
    )
$script:QuickAdoptionReusableReuses =
    [System.Collections.Generic.Dictionary[string, int]]::new(
        [StringComparer]::Ordinal
    )
$script:QuickAdoptionReusableIdentities =
    [System.Collections.Generic.Dictionary[string, object]]::new(
        [StringComparer]::Ordinal
    )
$script:QuickAdoptionReusableIdentityKeys =
    [System.Collections.Generic.Dictionary[string, string]]::new(
        [StringComparer]::Ordinal
    )
$script:QuickAdoptionReusableBuilders =
    [System.Collections.Generic.Dictionary[string, object]]::new(
        [StringComparer]::Ordinal
    )
$script:QuickAdoptionFreshManagedBuilds =
    [System.Collections.Generic.Dictionary[string, int]]::new(
        [StringComparer]::Ordinal
    )
$script:QuickAdoptionMutableDerivatives =
    [System.Collections.Generic.List[object]]::new()
$script:QuickAdoptionDerivativeRoots =
    [System.Collections.Generic.HashSet[string]]::new(
        [StringComparer]::OrdinalIgnoreCase
    )
$script:QuickAdoptionFixtureFamilyGitInitCount = 0
$script:QuickAdoptionProtocolOwnerBuildCount = 0
$script:QuickAdoptionCredentialContainmentGitProcessCount = 0
$script:QuickAdoptionGitHubCliVersionFullLauncherCount = 0
...
function New-MockRemoteEmptyCommit {
    ...
    Invoke-TestGit -Repository $Remote -BareRepository -Arguments @(
    ...
    $emptyTree = (@(Invoke-TestGit -Repository $Remote -BareRepository `
        -Arguments @('write-tree')))[0]
    return (@(Invoke-TestGit -Repository $Remote -BareRepository `
...
function Get-MockLiveDefaultBranch {
    ...
    $mainRefs = @(Invoke-TestGit `
        -Repository $global:QuickAdoptionRemotePath -BareRepository `
        -Arguments @(
...
function Invoke-QuickAdoptionFixtureFamilyGitInit {
    ...
    $script:QuickAdoptionFixtureFamilyGitInitCount++
    & git init @Arguments 2>&1 | Out-Null
...
function Get-QuickAdoptionFixtureEntryMode {
    ...
    if ($env:OS -ne 'Windows_NT') {
        ...
        if ($null -eq $method) {
            throw 'Unix fixture-mode evidence is unavailable on this runtime.'
        }
...
function Get-QuickAdoptionTreeFingerprint {
    param([Parameter(Mandatory)][string]$Path)

    $rootInfo = Get-Item -LiteralPath $Path -Force
    if (-not $rootInfo.PSIsContainer -or
        ($rootInfo.Attributes -band [IO.FileAttributes]::ReparsePoint)) {
        throw "Fixture fingerprint root '$Path' must be one real directory."
    }
    $rootPath = [IO.Path]::GetFullPath($rootInfo.FullName).TrimEnd(
        [IO.Path]::DirectorySeparatorChar,
        [IO.Path]::AltDirectorySeparatorChar
    )
    $entries = [System.Collections.Generic.List[string]]::new()
    $entries.Add(
        "ROOT`0$(Get-QuickAdoptionFixtureEntryMode -Item $rootInfo)"
    )
    foreach ($item in @(Get-ChildItem -LiteralPath $rootPath -Force -Recurse)) {
        if ($item.Attributes -band [IO.FileAttributes]::ReparsePoint) {
            throw "Fixture tree '$rootPath' contains linked or reparse entry '$($item.FullName)'."
        }
        $relativePath = $item.FullName.Substring($rootPath.Length).TrimStart(
            [IO.Path]::DirectorySeparatorChar,
            [IO.Path]::AltDirectorySeparatorChar
        ).Replace([IO.Path]::DirectorySeparatorChar, '/')
        $entryMode = Get-QuickAdoptionFixtureEntryMode -Item $item
        if ($item.PSIsContainer) {
            $entries.Add("D`0$relativePath`0$entryMode")
        }
        else {
            $fileHash = (Get-FileHash -LiteralPath $item.FullName `
                -Algorithm SHA256).Hash.ToLowerInvariant()
            $entries.Add(
                "F`0$relativePath`0$entryMode`0$($item.Length)`0$fileHash"
            )
        }
    }
...
function Assert-QuickAdoptionFingerprintModeSensitivity {
    if ($env:OS -eq 'Windows_NT') { return }

    $probeRoot = New-TempRoot -Name 'fingerprint-mode'
    $probePath = Join-Path $probeRoot 'mode-probe.txt'
    [IO.File]::WriteAllText(
        $probePath, "mode probe`n", [Text.UTF8Encoding]::new($false)
    )
    $before = Get-QuickAdoptionTreeFingerprint -Path $probeRoot
    try {
        & chmod u+x -- $probePath
        if ($LASTEXITCODE -ne 0) {
            throw 'Unable to create the quick-adoption mode-only mutation.'
        }
        $changed = Get-QuickAdoptionTreeFingerprint -Path $probeRoot
        if ($changed -ceq $before) {
            throw 'TEST-0158 quick-adoption fingerprint ignored a mode-only mutation.'
        }
    }
    finally {
        & chmod u-x -- $probePath
        if ($LASTEXITCODE -ne 0) {
            throw 'Unable to restore the quick-adoption mode-only mutation.'
        }
    }
    $restored = Get-QuickAdoptionTreeFingerprint -Path $probeRoot
    if ($restored -cne $before) {
        throw 'TEST-0158 quick-adoption mode-only mutation did not restore exactly.'
    }
}

function Copy-QuickAdoptionDirectoryTree {
    param(
    ...
    if (Test-Path -LiteralPath $Destination) {
        throw "Mutable derivative destination '$Destination' already exists."
    }
    $sourceInfo = Get-Item -LiteralPath $Source -Force
    if (-not $sourceInfo.PSIsContainer -or
        ($sourceInfo.Attributes -band [IO.FileAttributes]::ReparsePoint)) {
        throw "Mutable derivative source '$Source' must be one real directory."
    }
    $sourcePath = [IO.Path]::GetFullPath($sourceInfo.FullName).TrimEnd(
        [IO.Path]::DirectorySeparatorChar,
        [IO.Path]::AltDirectorySeparatorChar
    )
    ...
    foreach ($item in @(Get-ChildItem -LiteralPath $sourcePath -Force -Recurse)) {
        if ($item.Attributes -band [IO.FileAttributes]::ReparsePoint) {
            throw "Mutable derivative source '$sourcePath' contains linked or reparse entry '$($item.FullName)'."
        }
        ...
        if ($item.PSIsContainer) {
            ...
            [IO.File]::Copy($item.FullName, $destinationPath, $false)
        }
    }
}

function Assert-QuickAdoptionCopiedFileIsolation {
    param(
        [Parameter(Mandatory)][string]$OwnerPath,
        [Parameter(Mandatory)][string]$DerivativePath
    )

    $ownerBytes = [IO.File]::ReadAllBytes($OwnerPath)
    $derivativeBytes = [IO.File]::ReadAllBytes($DerivativePath)
    $ownerHashBefore = (Get-FileHash -LiteralPath $OwnerPath `
        -Algorithm SHA256).Hash
    $probeBytes = [byte[]]::new($derivativeBytes.Length + 1)
    [Array]::Copy(
        $derivativeBytes, 0, $probeBytes, 0, $derivativeBytes.Length
    )
    $probeBytes[$probeBytes.Length - 1] = 10
    try {
        [IO.File]::WriteAllBytes($DerivativePath, $probeBytes)
        $ownerHashAfter = (Get-FileHash -LiteralPath $OwnerPath `
            -Algorithm SHA256).Hash
        if ($ownerHashAfter -cne $ownerHashBefore -or
            [Convert]::ToBase64String(
                [byte[]][IO.File]::ReadAllBytes($OwnerPath)
            ) -cne [Convert]::ToBase64String($ownerBytes)) {
            throw "Mutable derivative file '$DerivativePath' shares storage with owner '$OwnerPath'."
        }
    }
    finally {
        [IO.File]::WriteAllBytes($DerivativePath, $derivativeBytes)
    }
}

function Copy-QuickAdoptionProtocolAssetSnapshots {
    param(
        [Parameter(Mandatory)]
        [Collections.Generic.IReadOnlyDictionary[string, object]]$Snapshots
    )

    $copies = [System.Collections.Generic.Dictionary[string, object]]::new(
        [StringComparer]::Ordinal
    )
    foreach ($entry in $Snapshots.GetEnumerator()) {
        $copies[[string]$entry.Key] = [pscustomobject]@{
            Bytes = [byte[]]$entry.Value.Bytes.Clone()
            Sha = [string]$entry.Value.Sha
        }
    }
...
function Get-QuickAdoptionReusableFixture {
    param(
        ...
        [object[]]$ArgumentList = @(),
        [Parameter(Mandatory)][string[]]$InputRecords
    )

    $builderIdentity = Get-QuickAdoptionStringSha256 `
        -Value ([string]$Builder.ToString())
    $ownerSourceDigest = $script:QuickAdoptionSuiteOwnerDigest
    $argumentIdentity = @($ArgumentList | ForEach-Object {
        if ($null -eq $_ -or $_ -isnot [string] -or
            [string]$_ -cmatch '[\r\n]') {
            throw "Reusable fixture '$Key' has a non-canonical builder argument."
        }
        [string]$_
    }) -join "`0"
    $canonicalInputRecords = @($InputRecords | ForEach-Object {
        if ([string]::IsNullOrWhiteSpace($_) -or
            [string]$_ -cnotmatch '^[a-z][a-z0-9.-]*=[^\r\n\x00]+$') {
            throw "Reusable fixture '$Key' has a non-canonical input record."
        }
        [string]$_
    })
    [Array]::Sort($canonicalInputRecords, [StringComparer]::Ordinal)
    if ($canonicalInputRecords.Count -eq 0 -or
        @($canonicalInputRecords | Select-Object -Unique).Count -ne
            $canonicalInputRecords.Count) {
        throw "Reusable fixture '$Key' must declare distinct exact input records."
    }
    $inputMaterial = @(
        'schema=2',
        "ownerSource=$ownerSourceDigest",
        "builder=$builderIdentity",
        "arguments=$argumentIdentity"
    ) + @($canonicalInputRecords | ForEach-Object {
        "input=$_"
    })
    $inputDigest = Get-QuickAdoptionStringSha256 `
        -Value ($inputMaterial -join "`n")
    $identityKey = "$builderIdentity`0$inputDigest"
    if ($script:QuickAdoptionReusableIdentities.ContainsKey($Key)) {
        $existingIdentity = $script:QuickAdoptionReusableIdentities[$Key]
        if ([string]$existingIdentity.Builder -cne $builderIdentity -or
            [string]$existingIdentity.InputDigest -cne $inputDigest) {
            throw ("Reusable fixture '$Key' conflicts with its canonical " +
                "builder or input digest: existing builder=" +
                "$([string]$existingIdentity.Builder), requested builder=" +
                "$builderIdentity, existing input=" +
                "$([string]$existingIdentity.InputDigest), requested input=" +
                "$inputDigest.")
        }
    }
    else {
        if ($script:QuickAdoptionReusableIdentityKeys.ContainsKey($identityKey)) {
            throw "Reusable fixture '$Key' aliases canonical key '$($script:QuickAdoptionReusableIdentityKeys[$identityKey])'."
        }
        $script:QuickAdoptionReusableIdentities[$Key] =
            [pscustomobject][ordered]@{
                Owner = 'tests/capabilities/initial-adoption/quick-adoption.tests.ps1'
                Key = $Key
                Builder = $builderIdentity
                OwnerSourceDigest = $ownerSourceDigest
                Scope = 'SuiteProcess'
                InputDigest = $inputDigest
                InputRecords = @($canonicalInputRecords)
            }
        $script:QuickAdoptionReusableBuilders[$Key] =
            [pscustomobject]@{
                Source = [string]$Builder.ToString()
                Arguments = @($ArgumentList)
                InputRecords = @($canonicalInputRecords)
            }
        $script:QuickAdoptionReusableIdentityKeys[$identityKey] = $Key
    }

    if (-not $script:QuickAdoptionReusableRequests.ContainsKey($Key)) {
        $script:QuickAdoptionReusableRequests[$Key] = 0
        $script:QuickAdoptionReusableBuilds[$Key] = 0
        $script:QuickAdoptionReusableReuses[$Key] = 0
    }
    $script:QuickAdoptionReusableRequests[$Key]++
    if ($script:QuickAdoptionReusableFixtures.ContainsKey($Key)) {
        $script:QuickAdoptionReusableReuses[$Key]++
        return $script:QuickAdoptionReusableFixtures[$Key]
    }

    $fixture = & $Builder @ArgumentList
    if ($null -eq $fixture -or
        -not (Test-Path -LiteralPath $fixture.Repository `
            -PathType Container) -or
        -not (Test-Path -LiteralPath $fixture.Remote -PathType Container)) {
        throw "Reusable fixture builder '$Key' did not produce one repository pair."
    }
    $fixture | Add-Member -NotePropertyName ReuseKey -NotePropertyValue $Key
    $fixture | Add-Member -NotePropertyName FixtureIdentity `
        -NotePropertyValue $script:QuickAdoptionReusableIdentities[$Key]
    $fixture | Add-Member -NotePropertyName ConsumerFingerprint `
        -NotePropertyValue (Get-QuickAdoptionTreeFingerprint `
            -Path $fixture.Repository)
    $fixture | Add-Member -NotePropertyName RemoteFingerprint `
        -NotePropertyValue (Get-QuickAdoptionTreeFingerprint `
            -Path $fixture.Remote)
    $script:QuickAdoptionReusableBuilds[$Key]++
    $script:QuickAdoptionReusableFixtures[$Key] = $fixture
    return $fixture
}

function New-QuickAdoptionMutableDerivative {
    param(
        [Parameter(Mandatory)]$OwnerFixture,
        [Parameter(Mandatory)][string]$Name
    )

    $derivativeRoot = New-TempRoot -Name "derivative-$Name"
    $rootPath = [IO.Path]::GetFullPath($derivativeRoot)
    if (-not $script:QuickAdoptionDerivativeRoots.Add($rootPath)) {
        throw "Mutable derivative root '$rootPath' was reused."
    }
    $repositoryPath = Join-Path $derivativeRoot 'consumer'
    $remotePath = Join-Path $derivativeRoot 'consumer.git'
    Copy-QuickAdoptionDirectoryTree -Source $OwnerFixture.Repository `
        -Destination $repositoryPath
    Copy-QuickAdoptionDirectoryTree -Source $OwnerFixture.Remote `
        -Destination $remotePath

    Assert-QuickAdoptionCopiedFileIsolation `
        -OwnerPath (Join-Path $OwnerFixture.Repository 'app.txt') `
        -DerivativePath (Join-Path $repositoryPath 'app.txt')
    Assert-QuickAdoptionCopiedFileIsolation `
        -OwnerPath (Join-Path $OwnerFixture.Remote 'config') `
        -DerivativePath (Join-Path $remotePath 'config')

    $slug = "test-owner/$Name"
    $ownerRemoteUrl = $OwnerFixture.Remote.Replace('\\', '/')
    $derivativeRemoteUrl = $remotePath.Replace('\\', '/')
    Invoke-TestGit -Repository $repositoryPath -Arguments @(
        'config', '--unset-all', "url.$ownerRemoteUrl.insteadOf"
    ) | Out-Null
    Invoke-TestGit -Repository $repositoryPath -Arguments @(
        'config', "url.$derivativeRemoteUrl.insteadOf",
    ...
    $configText = [IO.File]::ReadAllText(
        (Join-Path $repositoryPath '.git/config')
    )
    $configuredOrigin = (@(Invoke-TestGit -Repository $repositoryPath `
        -Arguments @('config', '--get', 'remote.origin.url')))[0]
    $configuredRewrite = (@(Invoke-TestGit -Repository $repositoryPath `
        -Arguments @(
            'config', '--get', "url.$derivativeRemoteUrl.insteadOf"
        )))[0]
    if ($configText.Contains([string]$OwnerFixture.Remote) -or
        $configText.Contains([string]$OwnerFixture.Slug) -or
        $configuredOrigin -cne "https://github.com/$slug.git" -or
        $configuredRewrite -cne "https://github.com/$slug.git") {
        throw "Mutable derivative '$Name' retained its owner remote binding."
    }
    foreach ($unsafePath in @(
        (Join-Path $repositoryPath '.git/objects/info/alternates'),
        (Join-Path $repositoryPath '.git/worktrees'),
        (Join-Path $remotePath 'objects/info/alternates'),
        (Join-Path $remotePath 'worktrees')
    )) {
        if (Test-Path -LiteralPath $unsafePath) {
            throw "Mutable derivative '$Name' contains shared Git state '$unsafePath'."
        }
    }

    if ([string]$OwnerFixture.Kind -ceq 'Managed') {
        $submodulePath = Join-Path $repositoryPath '.ai/protocol'
        $submoduleGitDirectory = (@(Invoke-TestGit `
            -Repository $submodulePath `
            -Arguments @('rev-parse', '--absolute-git-dir')))[0]
        $expectedModuleRoot = [IO.Path]::GetFullPath(
            (Join-Path $repositoryPath '.git/modules')
        ).TrimEnd([IO.Path]::DirectorySeparatorChar) +
            [IO.Path]::DirectorySeparatorChar
        $actualModulePath = [IO.Path]::GetFullPath(
            [string]$submoduleGitDirectory
        )
        if (-not $actualModulePath.StartsWith(
                $expectedModuleRoot,
                [StringComparison]::OrdinalIgnoreCase
            )) {
            throw "Mutable managed derivative '$Name' retained external submodule Git state '$actualModulePath'."
        }
    }

    $derivative = [pscustomobject]@{
        Root = $derivativeRoot
        Repository = $repositoryPath
        Remote = $remotePath
        Slug = $slug
        Kind = [string]$OwnerFixture.Kind
        ReuseKey = [string]$OwnerFixture.ReuseKey
    }
    $script:QuickAdoptionMutableDerivatives.Add($derivative)
    return $derivative
}

function Assert-QuickAdoptionFixtureReuse {
    param([switch]$RequireAllCounts)

    if ($script:QuickAdoptionFixtureFamilyGitInitCount -ne
        ($script:QuickAdoptionProtocolOwnerBuildCount +
         (2 * ($script:QuickAdoptionReusableBuilds.Values |
            Measure-Object -Sum).Sum) +
         (2 * ($script:QuickAdoptionFreshManagedBuilds.Values |
            Measure-Object -Sum).Sum))) {
        throw "Quick-adoption fixture-family git-init accounting is inconsistent: $($script:QuickAdoptionFixtureFamilyGitInitCount)."
    }
    foreach ($entry in $script:QuickAdoptionReusableFixtures.GetEnumerator()) {
        $fixture = $entry.Value
        $identity = $fixture.FixtureIdentity
        $consumerFingerprint = Get-QuickAdoptionTreeFingerprint `
            -Path $fixture.Repository
        $remoteFingerprint = Get-QuickAdoptionTreeFingerprint `
            -Path $fixture.Remote
        if ($consumerFingerprint -cne [string]$fixture.ConsumerFingerprint -or
            $remoteFingerprint -cne [string]$fixture.RemoteFingerprint -or
            [string]$identity.Owner -cne
                'tests/capabilities/initial-adoption/quick-adoption.tests.ps1' -or
            [string]$identity.Key -cne [string]$entry.Key -or
            [string]$identity.Builder -cnotmatch '^sha256:[0-9a-f]{64}$' -or
            [string]$identity.OwnerSourceDigest -cnotmatch
                '^sha256:[0-9a-f]{64}$' -or
            [string]$identity.Scope -cne 'SuiteProcess' -or
            [string]$identity.OwnerSourceDigest -cne
                $script:QuickAdoptionSuiteOwnerDigest -or
            [string]$identity.InputDigest -cnotmatch '^sha256:[0-9a-f]{64}$' -or
            @($identity.InputRecords).Count -eq 0 -or
            [int]$script:QuickAdoptionReusableBuilds[$entry.Key] -ne 1 -or
            [int]$script:QuickAdoptionReusableRequests[$entry.Key] -ne
                (1 + [int]$script:QuickAdoptionReusableReuses[$entry.Key])) {
            throw "Reusable fixture '$($entry.Key)' changed or has inconsistent build/reuse accounting."
        }
    }
    if ($script:QuickAdoptionMutableDerivatives.Count -ne
        $script:QuickAdoptionDerivativeRoots.Count) {
        throw 'Quick-adoption mutable derivative roots are not distinct.'
    }
    foreach ($derivative in $script:QuickAdoptionMutableDerivatives) {
        if (-not (Test-Path -LiteralPath $derivative.Root `
                -PathType Container) -or
            -not (Test-Path -LiteralPath $derivative.Repository `
                -PathType Container) -or
            -not (Test-Path -LiteralPath $derivative.Remote `
                -PathType Container)) {
            throw "Mutable derivative '$($derivative.Root)' leaked or lost one owned path before teardown."
        }
    }
    if (-not $RequireAllCounts) {
        return
    }
    ...
    foreach ($entry in $expectedRequests.GetEnumerator()) {
        if (-not $script:QuickAdoptionReusableRequests.ContainsKey($entry.Key) -or
            [int]$script:QuickAdoptionReusableRequests[$entry.Key] -ne
                [int]$entry.Value -or
            [int]$script:QuickAdoptionReusableBuilds[$entry.Key] -ne 1 -or
            [int]$script:QuickAdoptionReusableReuses[$entry.Key] -ne
                ([int]$entry.Value - 1)) {
            throw "Reusable fixture '$($entry.Key)' did not close at 1 build plus $([int]$entry.Value - 1) reuses."
        }
    }
    Assert-QuickAdoptionFingerprintModeSensitivity
    $canonicalBuilder =
        $script:QuickAdoptionReusableBuilders['connected-seed-workflow']
    $identityConflict = ''
    try {
        [void](Get-QuickAdoptionReusableFixture `
            -Key 'connected-seed-workflow' `
            -Builder { throw 'identity-conflict probe crossed its boundary' } `
            -InputRecords @($canonicalBuilder.InputRecords))
    }
    catch { $identityConflict = $_.Exception.Message }
    if ($identityConflict -cnotlike
            '*conflicts with its canonical builder or input digest*') {
        throw "Reusable fixture identity conflict was not rejected before its builder boundary: $identityConflict"
    }
    $inputConflict = ''
    try {
        [void](Get-QuickAdoptionReusableFixture `
            -Key 'connected-seed-workflow' `
            -Builder ([scriptblock]::Create([string]$canonicalBuilder.Source)) `
            -ArgumentList @($canonicalBuilder.Arguments) `
            -InputRecords @($canonicalBuilder.InputRecords + 'probe=changed'))
    }
    catch { $inputConflict = $_.Exception.Message }
    if ($inputConflict -cnotlike
            '*conflicts with its canonical builder or input digest*') {
        throw "Reusable fixture input drift was not rejected before its builder boundary: $inputConflict"
    }
    $aliasConflict = ''
    try {
        [void](Get-QuickAdoptionReusableFixture `
            -Key 'alias-connected-seed-workflow' `
            -Builder ([scriptblock]::Create([string]$canonicalBuilder.Source)) `
            -ArgumentList @($canonicalBuilder.Arguments) `
            -InputRecords @($canonicalBuilder.InputRecords))
    }
    catch { $aliasConflict = $_.Exception.Message }
    if ($aliasConflict -cnotlike '*aliases canonical key*') {
        throw "Reusable fixture alias was not rejected before its builder boundary: $aliasConflict"
    }
    if ($script:QuickAdoptionReusableRequests.Count -ne 3 -or
        $script:QuickAdoptionReusableIdentities.Count -ne 3 -or
        $script:QuickAdoptionReusableIdentityKeys.Count -ne 3 -or
        $script:QuickAdoptionReusableBuilders.Count -ne 3 -or
        $script:QuickAdoptionMutableDerivatives.Count -ne 21 -or
        $script:QuickAdoptionProtocolOwnerBuildCount -ne 1 -or
        ...
        -not $script:QuickAdoptionFreshManagedBuilds.ContainsKey('v0.16.1') -or
        [int]$script:QuickAdoptionFreshManagedBuilds['v0.9.2'] -ne 1 -or
        [int]$script:QuickAdoptionFreshManagedBuilds['v0.16.1'] -ne 1) {
...
function Copy-ImmutableProtocolPolicyFixture {
    param(
        [Parameter(Mandatory)][string]$Destination,
        [Parameter(Mandatory)][string]$Commit
    )

    $archiveRoot = New-TempRoot -Name 'immutable-policy-archive'
    $archivePath = Join-Path $archiveRoot 'policy.zip'
    ...
    try {
        Invoke-TestGit -Repository $root -Arguments (
            @('archive', '--format=zip', '-o', $archivePath, $Commit, '--') +
            $policyPaths
        ) | Out-Null
        Expand-Archive -LiteralPath $archivePath -DestinationPath $extractPath
        foreach ($policyPath in $policyPaths) {
            $relativePath = $policyPath -replace '/',
                [IO.Path]::DirectorySeparatorChar
            Copy-Item -LiteralPath (Join-Path $extractPath $relativePath) `
                -Destination (Join-Path $Destination $relativePath) -Force
        }
    }
    finally {
...
function Initialize-QuickAdoptionImmutableFixture {
    ...
    Invoke-QuickAdoptionFixtureFamilyGitInit `
        -Arguments @('-b', 'main', $protocolRepository) `
        -FailureMessage 'Unable to initialize the immutable mock protocol repository.'
    $script:QuickAdoptionProtocolOwnerBuildCount++
    ...
    Copy-CanonicalProtocolFixture -Destination $protocolRepository -Tag 'v0.14.1'
    ...
    Invoke-TestGit -Repository $protocolRepository -Arguments @(
        'commit', '-m', 'Create mock ancillary-fallback schema-1 protocol release'
    ) | Out-Null
    Invoke-TestGit -Repository $protocolRepository -Arguments @(
        'tag', 'v0.14.1'
    ) | Out-Null
    $ancillaryFallbackSha = (@(Invoke-TestGit -Repository $protocolRepository `
        -Arguments @('rev-parse', 'HEAD')))[0]
    $releaseCommits['v0.14.1'] = $ancillaryFallbackSha
    Save-MockProtocolAssetSnapshot -Tag 'v0.14.1' `
        -Repository $protocolRepository -Snapshots $assetSnapshots

    Copy-CanonicalProtocolFixture -Destination $protocolRepository -Tag 'v0.15.4'
    Copy-ImmutableProtocolPolicyFixture -Destination $protocolRepository `
        -Commit '1883a2315529e7493343c07eebb4c74ed77a62b4'
    Invoke-TestGit -Repository $protocolRepository -Arguments @(
        'add', '--', '.'
    ) | Out-Null
    Invoke-TestGit -Repository $protocolRepository -Arguments @(
        'commit', '-m', 'Create mock immutable schema-1 protocol release'
    ) | Out-Null
    Invoke-TestGit -Repository $protocolRepository -Arguments @(
        'tag', 'v0.15.4'
    ) | Out-Null
    $schema1Sha = (@(Invoke-TestGit -Repository $protocolRepository `
        -Arguments @('rev-parse', 'HEAD')))[0]
    $releaseCommits['v0.15.4'] = $schema1Sha
    Save-MockProtocolAssetSnapshot -Tag 'v0.15.4' `
        -Repository $protocolRepository -Snapshots $assetSnapshots
    ...
    Copy-ImmutableProtocolPolicyFixture -Destination $protocolRepository `
        -Commit '5321f1f1aa5966114c69b46bf6ed9191df109e6b'
    Invoke-TestGit -Repository $protocolRepository -Arguments @(
        'add', '--', '.'
    ) | Out-Null
    Invoke-TestGit -Repository $protocolRepository -Arguments @(
        'commit', '-m', 'Create mock immutable schema-2 predecessor release'
    ) | Out-Null
    Invoke-TestGit -Repository $protocolRepository -Arguments @(
        'tag', 'v0.15.6'
    ) | Out-Null
    $schema2PreviousSha = (@(Invoke-TestGit -Repository $protocolRepository `
        -Arguments @('rev-parse', 'HEAD')))[0]
    $releaseCommits['v0.15.6'] = $schema2PreviousSha
    ...
    $currentSha = (@(Invoke-TestGit -Repository $protocolRepository `
        -Arguments @('rev-parse', 'HEAD')))[0]
    $releaseCommits['v0.16.0'] = $currentSha
    Save-MockProtocolAssetSnapshot -Tag 'v0.16.0' `
        -Repository $protocolRepository -Snapshots $assetSnapshots
    ...
    $archivePath = Join-Path $protocolFixtureRoot 'protocol-v0.16.0.zip'
    Invoke-TestGit -Repository $protocolRepository -Arguments @(
        'archive', '--format=zip', '--prefix=openai-mock-protocol/',
        '-o', $archivePath, 'v0.16.0'
    ) | Out-Null
...
function Reset-Mocks {
    ...
    $global:QuickAdoptionProtocolAssetBytes =
        Copy-QuickAdoptionProtocolAssetSnapshots `
            -Snapshots $fixture.AssetSnapshots
...
function New-MockConnectedManagedConsumerPair {
    param(
        [Parameter(Mandatory)][string]$Name,
    ...
    Invoke-QuickAdoptionFixtureFamilyGitInit `
        -Arguments @('--bare', $remotePath) `
        -FailureMessage 'Unable to initialize the mock managed-consumer remote.'
    Invoke-QuickAdoptionFixtureFamilyGitInit `
        -Arguments @('-b', 'main', $repositoryPath) `
        -FailureMessage 'Unable to initialize the mock managed consumer.'
...
function New-MockConnectedManagedConsumer {
    ...
    if ($InstalledTag -cne 'v0.16.0') {
        if (-not $script:QuickAdoptionFreshManagedBuilds.ContainsKey(
                $InstalledTag
            )) {
            $script:QuickAdoptionFreshManagedBuilds[$InstalledTag] = 0
        }
        $script:QuickAdoptionFreshManagedBuilds[$InstalledTag]++
        return New-MockConnectedManagedConsumerPair `
            -Name $Name -InstalledTag $InstalledTag
    }

    $reuseKey = 'connected-managed-v0.16.0'
    $owner = Get-QuickAdoptionReusableFixture -Key $reuseKey -Builder {
        New-MockConnectedManagedConsumerPair `
            -Name 'owner-connected-managed-v0.16.0' `
            -InstalledTag 'v0.16.0'
    } -InputRecords @(
        'installed-tag=v0.16.0',
        "protocol-commit=$global:QuickAdoptionProtocolSha"
    )
    $consumer = New-QuickAdoptionMutableDerivative `
        -OwnerFixture $owner -Name $Name
    $global:QuickAdoptionRepoName = $consumer.Slug
    $global:QuickAdoptionTargetPath = $consumer.Repository
    $global:QuickAdoptionRemotePath = $consumer.Remote
    $global:QuickAdoptionDefaultBranch = 'main'
    $global:QuickAdoptionExistingSecrets.Clear()
    $global:QuickAdoptionExistingSecrets.Add('MEANDAI_UPDATER_TOKEN')
    $global:QuickAdoptionExistingSecrets.Add('MEANDAI_PROTOCOL_TOKEN')
    $global:QuickAdoptionPublishAdoptionProposal = $false
    $consumerHead = (@(Invoke-TestGit -Repository $consumer.Repository `
        -Arguments @('rev-parse', 'HEAD')))[0]
    $consumer | Add-Member -NotePropertyName Head `
        -NotePropertyValue $consumerHead
    return $consumer
}

function New-MockConnectedSeedConsumerPair {
    param(
        [Parameter(Mandatory)][string]$Name,
    ...
    Invoke-QuickAdoptionFixtureFamilyGitInit `
        -Arguments @('--bare', $remotePath) `
        -FailureMessage 'Unable to initialize the mock seed-consumer remote.'
    Invoke-QuickAdoptionFixtureFamilyGitInit `
        -Arguments @('-b', 'main', $repositoryPath) `
        -FailureMessage 'Unable to initialize the mock seed consumer.'
    Set-TestGitIdentity -Repository $repositoryPath
    [IO.File]::WriteAllText(
        (Join-Path $repositoryPath 'app.txt'),
        "consumer app`n",
...
function New-MockConnectedSeedConsumer {
    ...
    $reuseKey = if ($OmitWorkflowSeed) {
        'connected-seed-no-workflow'
    }
    else {
        'connected-seed-workflow'
    }
    $ownerName = "owner-$reuseKey"
    $owner = if ($OmitWorkflowSeed) {
        Get-QuickAdoptionReusableFixture -Key $reuseKey -Builder {
            param($name)
            New-MockConnectedSeedConsumerPair -Name $name -OmitWorkflowSeed
        } -ArgumentList @($ownerName) -InputRecords @(
            'workflow-seed=omitted'
        )
    }
    else {
        Get-QuickAdoptionReusableFixture -Key $reuseKey -Builder {
            param($name)
            New-MockConnectedSeedConsumerPair -Name $name
        } -ArgumentList @($ownerName) -InputRecords @(
            "workflow-blob=$global:QuickAdoptionWorkflowSha",
            'workflow-mode=100644'
        )
    }
    $consumer = New-QuickAdoptionMutableDerivative `
        -OwnerFixture $owner -Name $Name
    ...
    $global:QuickAdoptionTargetPath = $consumer.Repository
    $global:QuickAdoptionRemotePath = $consumer.Remote
    $env:MEANDAI_TEST_CODEX_TARGET = $consumer.Repository
    $env:MEANDAI_TEST_CODEX_REMOTE = $consumer.Remote
    return $consumer
}
...
function New-MockCompletedAdoptionConsumer {
    ...
    $completedBranch = 'automation/meandai-capabilities-v0.16.0'
...
function Initialize-MockAdoptionPullRequestBody {
    ...
    $bodySuffix = "`n`nMock adoption proposal."
    $markerRecord = if ($null -ne $sourceGraphIdentity) {
        [ordered]@{
            ...
            branch = 'automation/meandai-capabilities-v0.16.0'
        ...
        $surfaceLines = if (@($sourceGraphIdentity.protocolSurfaces).Count -gt 0) {
            @($sourceGraphIdentity.protocolSurfaces | ForEach-Object {
                $encodedPath = (@(([string]$_).Split('/') | ForEach-Object {
                    [Uri]::EscapeDataString([string]$_)
                }) -join '/')
                "- [``$_``](https://github.com/$($global:QuickAdoptionRepoName)/blob/$([string]$sourceGraphIdentity.graphBase)/$encodedPath)"
            })
        }
        else { @('- None') }
        $bodySuffix = (@(
            ...
            ''
        ) + @($surfaceLines) + @('', 'Mock adoption proposal.')) -join "`n"
    }
    else {
        $surfaceLines = if (@($global:QuickAdoptionProposalSurfaces).Count -gt 0) {
            @($global:QuickAdoptionProposalSurfaces | ForEach-Object {
                "- ``$_``"
            })
        }
        else { @('- None') }
        $bodySuffix = (@(
            ...
            '- None',
            '',
            'The transient proposal contract is stored in `.ai/adoption/meandai-capabilities.json`.',
            '',
            'Mock adoption proposal.'
        )) -join "`n"
        [ordered]@{
            schema = 5
            phase = 'Proposed'
    ...
    $global:QuickAdoptionPrBody =
        "<!-- meandai-capabilities-adoption:$marker -->`n$bodySuffix"
}
...
function New-TestQuickAdoptionPullRequestContractFixture {
    ...
    $pullRequest = [pscustomobject][ordered]@{
        ...
        headRefName = 'automation/meandai-capabilities-v0.16.0'
        headRefOid = $head
        baseRefName = 'main'
...
function Test-MockInitialPolicyAuthorityCall {
    ...
    return [string]$endpoint[0] -cin @(
        'repos/hasanmanzak/meAndAI/releases/tags/v0.16.0',
        'repos/hasanmanzak/meAndAI/commits/v0.16.0',
        'repos/hasanmanzak/meAndAI/contents/templates/project/.github/workflows/meandai-protocol-update.yml?ref=v0.16.0',
        'repos/hasanmanzak/meAndAI/contents/templates/project/.github/scripts/MeAndAI.CapabilitiesBootstrap.psm1?ref=v0.16.0'
    )
}
...
function Get-TestCanonicalAdoptionIssueMarker {
    param(
        [Parameter(Mandatory)][string]$Repository,
...
function global:gh {
    ...
    if ($Arguments.Count -ge 2 -and $Arguments[0] -eq 'issue' -and $Arguments[1] -eq 'create') {
        ...
        $createdIssue = [pscustomobject]@{
            ...
            title = 'Track meAndAI AI capabilities adoption from v0.16.0'
    ...
    if ($Arguments.Count -ge 3 -and $Arguments[0] -eq 'issue' -and
        ...
        if ($Arguments[1] -ceq 'edit') {
            $bodyIndex = [Array]::IndexOf(
                [object[]]$Arguments, '--body-file'
            )
            if ($bodyIndex -ge 0) {
                if ($bodyIndex + 1 -ge $Arguments.Count -or
                    -not (Test-Path -LiteralPath $Arguments[$bodyIndex + 1] `
                        -PathType Leaf)) {
                    throw 'Mock adoption issue body edit did not use one readable body file.'
                }
                $issue.body = [IO.File]::ReadAllText(
                    [string]$Arguments[$bodyIndex + 1]
                )
                $global:QuickAdoptionEvents.Add(
                    "issue-body-edit:$($issue.number)"
                )
            }
    ...
    if ($Arguments.Count -ge 2 -and $Arguments[0] -eq 'workflow' -and $Arguments[1] -eq 'run') {
        ...
        $jsonFlags = @($Arguments | Where-Object { [string]$_ -ceq '--json' })
        if ($jsonFlags.Count -ne 1 -or $Arguments -ccontains '--field' -or
            $Arguments -ccontains '--raw-field' -or
            [string]::IsNullOrWhiteSpace([string]$stdin)) {
            throw 'Mock workflow dispatch did not use one JSON stdin payload.'
        }
        try {
            $dispatchPayload = [string]$stdin | ConvertFrom-Json
        }
        catch {
            throw 'Mock workflow dispatch stdin is invalid JSON.'
        }
        if ($null -eq $dispatchPayload) {
            throw 'Mock workflow dispatch stdin did not contain an input object.'
        }
        foreach ($property in $dispatchPayload.PSObject.Properties) {
            if ($property.Name -cnotmatch '^[a-z_]+$' -or
                $property.Value -isnot [string] -or
                $dispatchFields.Contains($property.Name)) {
                throw 'Mock workflow dispatch contains an invalid, non-string, or duplicate input.'
            }
            $dispatchFields[$property.Name] = [string]$property.Value
        }
...
try {
    foreach ($fixtureContractFunction in @(
        ...
        'Get-QuickAdoptionReusableFixture',
        'New-QuickAdoptionMutableDerivative',
        'Assert-QuickAdoptionFixtureReuse'
    )) {
    ...
    if (Test-QuickAdoptionShard -Name 'ContractsPreflight') {
        ...
        $schema1WorkflowKey =
            "v0.15.4`ntemplates/project/.github/workflows/meandai-protocol-update.yml"
        $schema1WorkflowSnapshot =
            $global:QuickAdoptionProtocolAssetBytes[$schema1WorkflowKey]
        $schema1PolicyKey =
            "v0.15.4`ntemplates/project/.github/scripts/MeAndAI.CapabilitiesBootstrap.psm1"
        $schema1PolicySnapshot =
            $global:QuickAdoptionProtocolAssetBytes[$schema1PolicyKey]
        $ancillaryFallbackWorkflowKey =
            "v0.14.1`ntemplates/project/.github/workflows/meandai-protocol-update.yml"
        $ancillaryFallbackWorkflowSnapshot =
            $global:QuickAdoptionProtocolAssetBytes[$ancillaryFallbackWorkflowKey]
        $ancillaryFallbackPolicyKey =
            "v0.14.1`ntemplates/project/.github/scripts/MeAndAI.CapabilitiesBootstrap.psm1"
        ...
        try {
            $schema1PolicyContract = & $launcherPath `
                ...
                -SupportInput @([pscustomobject][ordered]@{
                    WorkflowBytes = [byte[]]$schema1WorkflowSnapshot.Bytes
            ...
            if ([string]$schema1PolicyContract.SelectedTag -cne 'v0.15.4' -or
                ...
                [int]$schema1PolicyContract.GraphSchema -ne 1 -or
                [int]$schema1PolicyContract.MaximumBlobBytes -ne 262144 -or
                [int]$schema1PolicyContract.AssetCalls -ne 1) {
                Add-Failure 'TEST-0153 graph-aware immutable v0.15.4 did not retain its release-owned schema-1 policy.'
            }
        }
        catch {
            Add-Failure (
        ...
        $dynamicPolicyModulesBefore = @(
            Get-Module | Where-Object {
                $_.Name -like 'MeAndAI.InitialAdoptionPolicy.*'
            } | ForEach-Object { [string]$_.Name } | Sort-Object
        )
        try {
            $ancillaryFallbackContract = & $launcherPath `
                -SupportAction 'InitialPolicyContract' `
                -SupportInput @([pscustomobject][ordered]@{
                    WorkflowBytes =
                        [byte[]]$ancillaryFallbackWorkflowSnapshot.Bytes
                    PolicyBytes =
                        [byte[]]$ancillaryFallbackPolicySnapshot.Bytes
                    PolicySha =
            ...
            $wrongTargetSources = @($targetSemanticCommands | Where-Object {
            ...
            if ([string]$ancillaryFallbackContract.SelectedTag -cne
                    'v0.14.1' -or
                [string]$ancillaryFallbackContract.ImportedTag -cne
                    'v0.14.1' -or
                [int]$ancillaryFallbackContract.GraphSchema -ne 1 -or
                [int]$ancillaryFallbackContract.MaximumBlobBytes -ne 262144 -or
                [int]$ancillaryFallbackContract.AssetCalls -ne 2 -or
                [int]$ancillaryFallbackContract.ModuleCount -ne 2 -or
                $wrongTargetSources.Count -ne 0 -or
                $wrongAncillarySources.Count -ne 0) {
                Add-Failure 'TEST-0153 graph-aware immutable v0.14.1 did not retain target graph semantics with bounded runtime ancillary helpers.'
            }
        }
        catch {
            Add-Failure (
                'TEST-0153 graph-aware immutable v0.14.1 policy composition ' +
                'failed: ' + $_.Exception.Message
            )
        }
        $ancillaryFallbackPolicyText = [Text.UTF8Encoding]::new(
            $false, $true
        ).GetString([byte[]]$ancillaryFallbackPolicySnapshot.Bytes)
        $partialAncillaryPolicyText = $ancillaryFallbackPolicyText.Replace(
            'Export-ModuleMember -Function @(',
            "function Get-MeAndAILinkedPathIdentityDigest { return ('0' * 64) }`n`nExport-ModuleMember -Function @("
        ).Replace(
            "    'Get-MeAndAIInstructionGraphLimits',",
            "    'Get-MeAndAIInstructionGraphLimits',`n    'Get-MeAndAILinkedPathIdentityDigest',"
        )
        if ($partialAncillaryPolicyText -ceq $ancillaryFallbackPolicyText -or
            -not $partialAncillaryPolicyText.Contains(
                'function Get-MeAndAILinkedPathIdentityDigest'
            )) {
            Add-Failure 'TEST-0153 partial ancillary-family mutation fixture was not constructed.'
        }
        else {
            [byte[]]$partialAncillaryPolicyBytes =
                [Text.UTF8Encoding]::new($false).GetBytes(
                    $partialAncillaryPolicyText
                )
            $partialAncillaryFailure = $null
            try {
                [void](& $launcherPath `
                    -SupportAction 'InitialPolicyContract' `
                    -SupportInput @([pscustomobject][ordered]@{
                        WorkflowBytes =
                            [byte[]]$ancillaryFallbackWorkflowSnapshot.Bytes
                        PolicyBytes = $partialAncillaryPolicyBytes
            ...
            catch { $partialAncillaryFailure = $_.Exception.Message }
            if ($partialAncillaryFailure -notlike
                    '*partial ancillary command family*') {
                Add-Failure (
                    'TEST-0153 partial target ancillary family did not fail ' +
                    'closed: ' + [string]$partialAncillaryFailure
                )
            }
        }
        $dynamicPolicyModulesAfter = @(
            Get-Module | Where-Object {
                $_.Name -like 'MeAndAI.InitialAdoptionPolicy.*'
            } | ForEach-Object { [string]$_.Name } | Sort-Object
        )
        if (($dynamicPolicyModulesAfter -join "`n") -cne
            ($dynamicPolicyModulesBefore -join "`n")) {
            Add-Failure 'TEST-0153 composed target/runtime policy modules leaked after exact cleanup.'
        }
        $schema1PolicyText = [Text.UTF8Encoding]::new(
            $false, $true
        ).GetString([byte[]]$schema1PolicySnapshot.Bytes)
        $mutatedSchema1PolicyText = $schema1PolicyText.Replace(
            '$script:MeAndAIInstructionGraphMaximumNodes = 512',
            '$script:MeAndAIInstructionGraphMaximumNodes = 511'
        )
        if ($mutatedSchema1PolicyText -ceq $schema1PolicyText) {
            Add-Failure 'TEST-0153 retained-limit mutation fixture did not change the immutable policy.'
        }
        else {
            [byte[]]$mutatedSchema1PolicyBytes =
                [Text.UTF8Encoding]::new($false).GetBytes(
                    $mutatedSchema1PolicyText
                )
            $mutatedSchema1PolicyFailure = $null
            try {
                [void](& $launcherPath `
                    -SupportAction 'InitialPolicyContract' `
                    -SupportInput @([pscustomobject][ordered]@{
                        WorkflowBytes = [byte[]]$schema1WorkflowSnapshot.Bytes
                        PolicyBytes = $mutatedSchema1PolicyBytes
                        PolicySha = Get-TestQuickAdoptionGitBlobSha1 `
            ...
            if ($mutatedSchema1PolicyFailure -notlike
                    '*invalid instruction-graph limits*') {
                Add-Failure (
                    'TEST-0153 accepted drift in a retained immutable graph ' +
                    'limit: ' + [string]$mutatedSchema1PolicyFailure
                )
            }
        }
        Reset-Mocks
        ...
        $assetIsolationKey = "v0.16.0`n$($canonicalInitialAdoptionPolicyAsset.TemplatePath)"
        $firstAssetBytes = [byte[]]$global:QuickAdoptionProtocolAssetBytes[
            $assetIsolationKey
        ].Bytes
        $firstAssetBytes[0] = $firstAssetBytes[0] -bxor 1
        ...
        Reset-Mocks
        $secondAssetBytes = [byte[]]$global:QuickAdoptionProtocolAssetBytes[
            $assetIsolationKey
        ].Bytes
        $canonicalAssetBytes = [byte[]]$script:QuickAdoptionProtocolFixture.
            AssetSnapshots[$assetIsolationKey].Bytes
    ...
    if (@($launcherSourcePaths | Where-Object {
        ...
        if ($policyImportFunctions.Count -ne 1 -or
            ...
            if (-not $policyImportText.Contains('Get-CanonicalProtocolAsset') -or
                ...
                -not $policyImportText.Contains('-Tag $Tag') -or
                ...
                -not $policyImportText.Contains('Get-MeAndAIProtocolAssessmentLimits') -or
                -not $policyImportText.Contains('$schemaProbe') -or
                -not $policyImportText.Contains('GraphSchema = $graphSchema')) {
        ...
        $workflowIndex = $mainExecution.IndexOf(
            'Get-CanonicalWorkflow', [StringComparison]::Ordinal
        )
        $policyTagSelectionIndex = $mainExecution.IndexOf(
            'Resolve-QuickAdoptionInitialPolicyTag',
        ...
        if ($mainExecutionIndex -lt 0 -or $tokenReadIndex -lt 0 -or
            $authIndex -lt 0 -or $workflowIndex -lt 0 -or
            $policyTagSelectionIndex -lt 0 -or $policyImportIndex -lt 0 -or
            $assessmentIndex -lt 0 -or $tokenReadIndex -gt $authIndex -or
            $authIndex -gt $workflowIndex -or
            $workflowIndex -gt $policyTagSelectionIndex -or
            $policyTagSelectionIndex -gt $policyImportIndex -or
            $policyImportIndex -gt $assessmentIndex) {
            Add-Failure 'TEST-0130 token read, gh authentication, target-workflow policy selection, canonical module import, and assessment are not ordered at the mutation-free boundary.'
        }
    ...
    if (Test-Path -LiteralPath $mockCodexScriptPath -PathType Leaf) {
        ...
        if (-not $mockCodex.Contains('automation/meandai-capabilities-v0.16.0')) {
    ...
    foreach ($versionCase in @(
        ...
        $contractResult = & $contractAdapterPath `
            -SupportAction GitHubCliVersionContract `
            -SupportInput @($versionCase.Output)
        $contractCalls = @($contractResult.Calls)
        $contractError = [string]$contractResult.Error
        if ($contractCalls.Count -ne 1 -or
            [string]$contractCalls[0].Command -cne 'gh' -or
            (@($contractCalls[0].Arguments) -join ' ') -cne '--version') {
            Add-Failure "TEST-0107 $($versionCase.Name) production contract did not make exactly one gh --version call."
            continue
        }
        if ([bool]$versionCase.Accepted) {
            if (-not [bool]$contractResult.Succeeded -or $contractError) {
                Add-Failure "TEST-0107 $($versionCase.Name) production contract rejected a compatible version: $contractError"
            }
        }
        elseif ([bool]$contractResult.Succeeded -or
            $contractError -notlike '*GitHub CLI 2.82.1 or newer is required*' -or
            $contractError -notlike '*https://cli.github.com/*') {
            Add-Failure "TEST-0107 $($versionCase.Name) production contract did not fail with actionable minimum-version guidance: $contractError"
        }

        if ([string]$versionCase.Name -cnotin @('older', 'exact-floor')) {
            continue
        }
        ...
        $versionError = ''
        $script:QuickAdoptionGitHubCliVersionFullLauncherCount++
        try {
        ...
        if ($versionCalls.Count -ne 1 -or $nonVersionCalls.Count -ne 0 -or
            ...
            Add-Failure "TEST-0107 $($versionCase.Name) full launcher did not remain at the single version-query boundary."
            continue
        }
        if ([bool]$versionCase.Accepted) {
            if ($versionError -notlike '*TargetPath must identify an existing directory*') {
                Add-Failure "TEST-0107 $($versionCase.Name) full launcher did not continue to target validation: $versionError"
            }
        }
        elseif ($versionError -notlike '*GitHub CLI 2.82.1 or newer is required*' -or
            $versionError -notlike '*https://cli.github.com/*') {
            Add-Failure "TEST-0107 $($versionCase.Name) full launcher did not fail with actionable minimum-version guidance: $versionError"
        }
    }
    if ($script:QuickAdoptionGitHubCliVersionFullLauncherCount -ne
        $githubCliVersionFullLauncherMaximum) {
        Add-Failure 'TEST-0107 full-launcher operation count differs from the reviewed maximum.'
    }
    Confirm-MeAndAIScenarioEvidence -Context $scenarioContext `
        -TestId 'TEST-0107'
    ...
    if ($failures.Count -eq 0) {
        ...
        $credentialProbe = & $contractAdapterPath `
            -SupportAction CredentialContainmentContract `
            -TargetPath $historyRepo
        $script:QuickAdoptionCredentialContainmentGitProcessCount =
            [long]$credentialProbe.GitProcessCount
        $credentialPathspecs = @(
            ':(icase,glob)**/FG_PAT.txt',
            ':(icase,glob)**/MEANDAI_RO_FG_PAT.txt'
        )
        $expectedCredentialCalls = @(
            'rev-parse|--is-shallow-repository',
            ('ls-files|--stage|--|' + ($credentialPathspecs -join '|')),
            ('log|--all|--reflog|--format=%H|--|' +
                ($credentialPathspecs -join '|'))
        )
        $actualCredentialCalls = @($credentialProbe.Calls | ForEach-Object {
            @($_.Arguments) -join '|'
        })
        if (-not [bool]$credentialProbe.Succeeded -or
            [string]$credentialProbe.Error -or
            $script:QuickAdoptionCredentialContainmentGitProcessCount -ne
                $credentialContainmentGitProcessMaximum -or
            ($actualCredentialCalls -join "`n") -cne
                ($expectedCredentialCalls -join "`n")) {
            Add-Failure ('TEST-0159 credential-containment operation probe differs: ' +
                "count=$($script:QuickAdoptionCredentialContainmentGitProcessCount); " +
                "error=$([string]$credentialProbe.Error); " +
                "calls=$($actualCredentialCalls -join ', ')")
        }
    ...
    if ((Test-QuickAdoptionShard -Name 'AdoptionLifecycle') -and
        ...
        $publishedHead = ((& git -C $existingRepo rev-parse HEAD) -join '').Trim()
        function New-QuickAdoptionDispatchJson {
            param(
                [string]$CorrelationId = ('0' * 32),
                [string]$ExpectedBaseSha = $publishedHead
            )
            [ordered]@{
                correlation_id = $CorrelationId
                adoption_strategy = 'FreshAdoption'
                acknowledge_protocol_record_loss = 'false'
                expected_base_sha = $ExpectedBaseSha
            } | ConvertTo-Json -Compress
        }
        $wrongDispatches = @(
            [pscustomobject]@{
                Arguments = @('workflow', 'run', 'wrong-workflow.yml', '--repo',
                    'test-owner/consumer', '--ref', 'main', '--json')
                Stdin = New-QuickAdoptionDispatchJson
            },
            [pscustomobject]@{
                Arguments = @('workflow', 'run', 'meandai-protocol-update.yml',
                    '--repo', 'other/consumer', '--ref', 'main', '--json')
                Stdin = New-QuickAdoptionDispatchJson
            },
            [pscustomobject]@{
                Arguments = @('workflow', 'run', 'meandai-protocol-update.yml',
                    '--repo', 'test-owner/consumer', '--ref', 'develop', '--json')
                Stdin = New-QuickAdoptionDispatchJson
            },
            [pscustomobject]@{
                Arguments = @('workflow', 'run', 'meandai-protocol-update.yml',
                    '--repo', 'test-owner/consumer', '--ref', 'main', '--json')
                Stdin = New-QuickAdoptionDispatchJson `
        ...
        foreach ($wrongDispatch in $wrongDispatches) {
            $rejected = $false
            try {
                $wrongArguments = [object[]]$wrongDispatch.Arguments
                [string]$wrongDispatch.Stdin | & gh @wrongArguments | Out-Null
            }
            catch {
                $rejected = $true
            }
            if (-not $rejected) {
                Add-Failure "TEST-0090 mock accepted wrong workflow dispatch identity: $($wrongArguments -join ' ')"
            }
        }
        ...
        $initialPolicyRestCalls = @($global:QuickAdoptionRestCalls |
            Where-Object {
                [string]$_.Uri -ceq
                    'https://api.github.com/repos/hasanmanzak/meAndAI/contents/templates/project/.github/scripts/MeAndAI.CapabilitiesBootstrap.psm1?ref=v0.16.0'
            })
        $workflowRestCalls = @($global:QuickAdoptionRestCalls |
            Where-Object {
                [string]$_.Uri -ceq
                    'https://api.github.com/repos/hasanmanzak/meAndAI/contents/templates/project/.github/workflows/meandai-protocol-update.yml?ref=v0.16.0'
            })
        ...
        $canonicalIssueMarker = Get-TestCanonicalAdoptionIssueMarker `
            -Repository $global:QuickAdoptionRepoName -TargetTag 'v0.16.0' `
            -ProtocolSha $global:QuickAdoptionProtocolSha `
        ...
        if ($openMarkedIssues.Count -ne 1 -or [int]$openMarkedIssues[0].number -ne 83 -or
            ...
            Add-Failure 'TEST-0069 exact issue ownership did not converge without claiming an unmarked quoted note.'
        }
        $expectedVisiblePullRequestLink =
            "- Adoption draft: [PR #42](https://github.com/$($global:QuickAdoptionRepoName)/pull/42)"
        $issueGraphIdentity =
            $global:QuickAdoptionDispatchedSourceGraphIdentity
        ...
        $expectedIssueReferences = @(
            "- Protocol release: [v0.16.0](https://github.com/hasanmanzak/meAndAI/releases/tag/v0.16.0)",
        ...
        if (-not ([string]$openMarkedIssues[0].body).Contains(
                $expectedVisiblePullRequestLink
            ) -or
        ...
        $trackingLines = [regex]::Matches(
            ...
            '^Tracking issue: \[#83\]\(https://github\.com/' +
                [regex]::Escape($global:QuickAdoptionRepoName) +
                '/issues/83\)\r?$',
        ...
        if ($trackingLines.Count -ne 1 -or $closingIssueReferences.Count -ne 0 -or
            [regex]::IsMatch(
                [string]$global:QuickAdoptionPrBody,
                '(?m)^Tracking issue: #83\r?$'
            )) {
        ...
        if (-not $contentionBlocked -or
            ...
            Add-Failure 'TEST-0070 deterministic preseeded contention state did not fail closed before secret inventory and writes.'
        }
        [void]$global:QuickAdoptionLabelRecords.Remove('meandai:secret-reconciliation-lock')
        ...
        if (-not $ownerChangeBlocked -or
            ...
            -not $global:QuickAdoptionLabelRecords.ContainsKey('meandai:secret-reconciliation-lock')) {
            Add-Failure 'TEST-0070 deterministic ownership-change state was deleted or accepted instead of failing closed.'
        }
        Confirm-MeAndAIScenarioEvidence -Context $scenarioContext `
            -TestId 'TEST-0070'
        [void]$global:QuickAdoptionLabelRecords.Remove('meandai:secret-reconciliation-lock')
        ...
        $adoptionPaths = @(Invoke-Git -Repository $existingRemote -Arguments @(
            'ls-tree', '-r', '--name-only', 'refs/heads/automation/meandai-capabilities-v0.16.0'
        ))
        ...
        $protocolSourceEndpoint = 'repos/hasanmanzak/meAndAI/contents/templates/project/.github/workflows/meandai-protocol-update.yml?ref=v0.16.0'
        ...
        $initialPolicySourceEndpoint = 'repos/hasanmanzak/meAndAI/contents/templates/project/.github/scripts/MeAndAI.CapabilitiesBootstrap.psm1?ref=v0.16.0'
        ...
        $interruptedRemoteHead = (@(Invoke-Git -Repository $existingRepo -Arguments @(
            'ls-remote', '--heads', 'origin',
            'refs/heads/automation/meandai-capabilities-v0.16.0'
        )))[0].Split("`t")[0]
    ...
    if (Test-QuickAdoptionShard -Name 'InstructionGraphClosure') {
        ...
        $graphAwareBranch = 'automation/meandai-capabilities-v0.16.0'
        ...
        $graphAwareSurfaceLines = @($sourceGraph.protocolSurfaces |
            ForEach-Object {
                $encodedPath = (@(([string]$_).Split('/') | ForEach-Object {
                    [Uri]::EscapeDataString([string]$_)
                }) -join '/')
                "- [``$_``](https://github.com/$graphAwareRepository/blob/$([string]$sourceGraphIdentity.graphBase)/$encodedPath)"
            })
        $graphAwareBody = ((@(
            "<!-- meandai-capabilities-adoption:$graphAwareMarkerJson -->",
            '### Detected protocol and governance surfaces',
            ''
        ) + @($graphAwareSurfaceLines)) -join "`n")
        ...
        $graphAwareMarkerValid = & $markerValidator `
            ...
            -BaseBranch 'main' -TargetTag 'v0.16.0' `
        ...
        if (-not $graphAwareMarkerValid) {
            Add-Failure 'TEST-0154 graph-aware local-completion regression fixture did not start from one exact path-free schema-9 source-graph marker with linked surfaces.'
        }
    ...
    if ($runIntegrityShards -and
        ...
        $validManifestAccepted = & $manifestValidator `
            ...
            -TargetTag 'v0.16.0' -ProtocolSha ('a' * 40) `
        ...
        if ($canonicalOwnedIssue.Count -ne 1) {
            Add-Failure 'TEST-0069 marker-variant fixture could not locate its canonical adoption issue.'
        }
        else {
            ...
            $markerVariants = [ordered]@{
                malformed = $canonicalOwnedBody.Replace(
                    ...
                    $canonicalIssueMarker.Substring(
                        0, $canonicalIssueMarker.Length - 1
                    )
                )
                duplicate = "$canonicalOwnedBody`n$canonicalIssueMarker"
                'marker-only' = $canonicalIssueMarker
                'quoted-marker' = "> $canonicalIssueMarker`nUntrusted quotation"
                'second-line-marker' = "Untrusted preface`n$canonicalOwnedBody"
                'leading-space-marker' = $canonicalOwnedBody.Replace(
                    $canonicalIssueMarker, " $canonicalIssueMarker"
                )
                'wrong-visible-pr-target' = $canonicalOwnedBody.Replace(
                    "/pull/42)", '/pull/41)'
                )
                'raw-pr-reference' = $canonicalOwnedBody.Replace(
            ...
            foreach ($variant in $markerVariants.GetEnumerator()) {
                ...
                if (-not $markerVariantBlocked -or
                    ...
                    Add-Failure "TEST-0069/TEST-0176 $($variant.Key) adoption-issue marker did not fail closed before issue mutation and Codex execution."
                }
                $markerVariantIndex++
            }
        }
        if ($canonicalOwnedIssue.Count -eq 1) {
            ...
            Publish-MockAdoptionBranch
            $legacyIssueMarker =
                '<!-- meandai-local-adoption:v0.16.0:pr-42 -->'
            $pullRequestUrl =
            ...
            $legacyOwnedBody = $legacyOwnedBody.Replace(
                ...
                "- Source graph base: [$([string]$graphIdentity.graphBase)](https://github.com/$($global:QuickAdoptionRepoName)/commit/$([string]$graphIdentity.graphBase))",
                "- Source graph base: ``$([string]$graphIdentity.graphBase)``"
            )
            foreach ($surface in @($graphIdentity.protocolSurfaces)) {
                $encodedSurface = (@(([string]$surface).Split('/') |
                    ForEach-Object {
                        [Uri]::EscapeDataString([string]$_)
                    }) -join '/')
                $legacyOwnedBody = $legacyOwnedBody.Replace(
                    "- [``$surface``](https://github.com/$($global:QuickAdoptionRepoName)/blob/$([string]$graphIdentity.graphBase)/$encodedSurface)",
                    "- ``$surface``"
                )
            }
            $global:QuickAdoptionIssues.Clear()
            $legacyIssue = [pscustomobject]@{
                number = 83
                url = "https://github.com/$($global:QuickAdoptionRepoName)/issues/83"
                title = [string]$canonicalOwnedIssue[0].title
                body = $legacyOwnedBody
                state = 'OPEN'
            }
            $global:QuickAdoptionIssues.Add($legacyIssue)
            $global:QuickAdoptionIssue = $legacyIssue
            ...
            if ($legacyConvergenceFailed -or
                $legacyEditCountAfter -ne $legacyEditCountBefore + 1 -or
                -not ([string]$legacyIssue.body).StartsWith(
                    $canonicalIssueMarker, [StringComparison]::Ordinal
                ) -or
                -not ([string]$legacyIssue.body).Contains(
                    "- Adoption draft: [PR #42]($pullRequestUrl)"
                ) -or
                ...
                switch ($localCaseName) {
                    ...
                    'current-competing' {
                        $currentCompetingMarker =
                            '<!-- meandai-local-adoption:v0.16.0:pr-41 -->'
                        $localIssue.title =
                            'Track meAndAI AI capabilities adoption from v0.16.0'
                        $localIssue.body = ([string]$localIssue.body).Replace(
                            $historicalMarker, $currentCompetingMarker
                        ).Replace('v0.9.2', 'v0.16.0')
                    }
                    'newer-competing' {
                        $newerMarker =
                            '<!-- meandai-local-adoption:v0.16.1:pr-41 -->'
                        $localIssue.title =
                            'Track meAndAI AI capabilities adoption from v0.16.1'
                        $localIssue.body = ([string]$localIssue.body).Replace(
                            $historicalMarker, $newerMarker
                        ).Replace('v0.9.2', 'v0.16.1')
                    }
    ...
    if ($runIntegrityShards -and
        ...
        if (-not $timeoutBlocked -or $global:QuickAdoptionPrReadyCalls -ne 0 -or
            ...
            Add-Failure "TEST-0066 local Codex timeout path did not terminate cleanly before readiness (ready=$($global:QuickAdoptionPrReadyCalls); remoteHeadUnchanged=$($timeoutRemoteHeadAfter -ceq $timeoutRemoteHeadBefore); newTempRoots=$($newTimeoutTempRoots.Count); error=$timeoutMessage)."
        }
        $env:MEANDAI_TEST_CODEX_MODE = 'Success'
    ...
    if ($runIntegrityShards -and
        ...
        $collisionEntry = @((Invoke-Git -Repository $existingRemote -Arguments @(
            'ls-tree', 'refs/heads/automation/meandai-capabilities-v0.16.0', '--', '.ai/protocol'
        )))
        ...
        $migrationBranchApplicationBlob = (@(Invoke-TestGit `
            -Repository $existingRemote -Arguments @(
                'rev-parse',
                'refs/heads/automation/meandai-capabilities-v0.16.0:app.txt'
            )))[0]
        $migrationAgentsContent = @(Invoke-TestGit `
            -Repository $existingRemote -Arguments @(
                'show',
                'refs/heads/automation/meandai-capabilities-v0.16.0:AGENTS.md'
            )) -join "`n"
        ...
        $missingLinkedMigrationSurfaces = @($expectedMigrationSurfaces |
            Where-Object {
                $encodedPath = (@(([string]$_).Split('/') | ForEach-Object {
                    [Uri]::EscapeDataString([string]$_)
                }) -join '/')
                $expectedLink = "[``$_``](https://github.com/$($global:QuickAdoptionRepoName)/blob/$migrationPublishedBaseSha/$encodedPath)"
                -not ([string]$global:QuickAdoptionPrBody).Contains($expectedLink) -or
                    -not $migrationIssueBody.Contains($expectedLink)
            })
    ...
    if ((Test-QuickAdoptionShard -Name 'RepositoryRoutes') -and
        ...
        if ($modifiedSeedError -notlike
                '*not the exact canonical v0.16.0 file*no GitHub repository or remote was changed*' -or
        ...
        $currentConsumer = New-MockConnectedManagedConsumer `
            -Name 'managed-current' -InstalledTag 'v0.16.0'
        ...
        foreach ($blockedRoute in @(
            [pscustomobject]@{ Name = 'manifest'; InstalledTag = 'v0.16.0'; TargetTag = 'v0.16.0'; Mutation = 'Manifest' },
            [pscustomobject]@{ Name = 'partial'; InstalledTag = 'v0.16.0'; TargetTag = 'v0.16.0'; Mutation = 'Partial' },
            [pscustomobject]@{ Name = 'missing-gitlink'; InstalledTag = 'v0.16.0'; TargetTag = 'v0.16.0'; Mutation = 'MissingGitlink' },
            [pscustomobject]@{ Name = 'drift'; InstalledTag = 'v0.16.0'; TargetTag = 'v0.16.0'; Mutation = 'Drift' },
            [pscustomobject]@{ Name = 'newer'; InstalledTag = 'v0.16.1'; TargetTag = 'v0.16.0'; Mutation = 'None' },
            [pscustomobject]@{ Name = 'cross-major'; InstalledTag = 'v0.16.0'; TargetTag = 'v1.0.0'; Mutation = 'None' }
        )) {
            Reset-Mocks
    ...
    if ($failures.Count -eq 0) {
        try {
            Assert-QuickAdoptionFixtureReuse `
                -RequireAllCounts:($Shard -ceq 'All')
        }
        catch {
            Add-Failure "TEST-0158/TEST-0159 quick-adoption reusable fixture teardown validation failed: $($_.Exception.Message)"
        }
    }
    if ($null -ne $script:QuickAdoptionProtocolFixture) {
        try {
    ...
    $survivingFixtureRoots = @($tempRoots | Where-Object {
        Test-Path -LiteralPath $_
    })
    if ($survivingFixtureRoots.Count -ne 0) {
...
if (Test-QuickAdoptionShard -Name 'CurrentLauncherRecovery') {
    ...
    $temporaryRootCleanupFunctions = @($launcherAst.FindAll({
        param($node)
        $node -is [Management.Automation.Language.FunctionDefinitionAst] -and
        $node.Name -ceq 'Remove-QuickAdoptionTemporaryRoot'
    }, $true))
    if ($recoveryFunctions.Count -ne 1) {
        Add-Failure 'TEST-0126 launcher must define one current-launcher recovery boundary.'
    }
    if ($temporaryRootCleanupFunctions.Count -ne 1) {
        Add-Failure 'TEST-0126 launcher must define one canonical temporary-root cleanup boundary.'
    }
    else {
    ...
    if ($recoveryFunctions.Count -eq 1 -and
        $temporaryRootCleanupFunctions.Count -eq 1) {
...
'@
            ...
            $recoveryModule = New-Module `
                ...
                -ArgumentList @(
                    ...
                    $temporaryRootCleanupFunctions[0].Extent.Text,
                    $consumerRemote,
                    $protocolSeed
                ) `
                -ScriptBlock {
                    param(
                        ...
                        [string]$TemporaryRootCleanupDefinition,
                    ...
                    function Invoke-External {
                        ...
                        $cloneArguments = @($cloneSource, $Arguments[3])
                        if ($separator -ge 0 -and
                            $separator + 1 -lt $Arguments.Count) {
                            $cloneArguments += @($Arguments[($separator + 1)..(
                                $Arguments.Count - 1
                            )])
                        }
                        $previousPreference = $ErrorActionPreference
                        $ErrorActionPreference = 'Continue'
                        try {
                            $output = @(& git clone @cloneArguments 2>&1)
                            $exitCode = $LASTEXITCODE
                        }
                        finally {
                            $ErrorActionPreference = $previousPreference
                    ...
                    Invoke-Expression $TemporaryRootCleanupDefinition
            ...
            if (Test-Path -LiteralPath $recoveryTestRoot) {
                Add-Failure "TEST-0158 quick-adoption recovery fixture cleanup leaked root '$recoveryTestRoot'."
            }
        }
    }
    Confirm-MeAndAIScenarioEvidence -Context $scenarioContext `
...
if ($null -ne $script:QuickAdoptionTestRepositoryModule) {
    Remove-Module -ModuleInfo $script:QuickAdoptionTestRepositoryModule -Force `
        -ErrorAction SilentlyContinue
    $script:QuickAdoptionTestRepositoryModule = $null
}
...
if ($Shard -ceq 'All') {
    ...
    $operationLine = Format-MeAndAITestOperationObservation `
        ...
        -Runtime $operationExpectation.Runtime -Counters @(
            [ordered]@{
                name = 'credential-containment.git-process-per-scan'
                actual = [long]$script:QuickAdoptionCredentialContainmentGitProcessCount
                maximum = [long]$credentialContainmentGitProcessMaximum
            },
            [ordered]@{
                name = 'github-cli-version.full-launcher'
                actual = [long]$script:QuickAdoptionGitHubCliVersionFullLauncherCount
                maximum = [long]$githubCliVersionFullLauncherMaximum
            },
            [ordered]@{
                name = 'reusable-fixture-family.init'
                actual = [long]$script:QuickAdoptionFixtureFamilyGitInitCount
                maximum = [long](Get-QuickAdoptionOperationMaximum `
                    -Name 'reusable-fixture-family.init')
            }
        )
    ...
    if (-not $operationRecord.Valid) {
        throw "Quick-adoption operation evidence is invalid: $($operationRecord.Message)"
    }
