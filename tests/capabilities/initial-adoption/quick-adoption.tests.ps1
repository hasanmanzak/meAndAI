...
function Assert-QuickAdoptionFixtureReuse {
    ...
    if ($script:QuickAdoptionReusableRequests.Count -ne 3 -or
        ...
        -not $script:QuickAdoptionFreshManagedBuilds.ContainsKey('v0.15.3') -or
        [int]$script:QuickAdoptionFreshManagedBuilds['v0.9.2'] -ne 1 -or
        [int]$script:QuickAdoptionFreshManagedBuilds['v0.15.3'] -ne 1) {
...
function Initialize-QuickAdoptionImmutableFixture {
    ...
    Copy-CanonicalProtocolFixture -Destination $protocolRepository -Tag 'v0.15.2'
    Invoke-TestGit -Repository $protocolRepository -Arguments @(
        'add', '--', '.'
    ) | Out-Null
    Invoke-TestGit -Repository $protocolRepository -Arguments @(
    ...
    $currentSha = (@(Invoke-TestGit -Repository $protocolRepository `
        -Arguments @('rev-parse', 'HEAD')))[0]
    $releaseCommits['v0.15.2'] = $currentSha
    Save-MockProtocolAssetSnapshot -Tag 'v0.15.2' `
        -Repository $protocolRepository -Snapshots $assetSnapshots

    Invoke-TestGit -Repository $protocolRepository -Arguments @(
        'switch', '--detach'
    ) | Out-Null
    foreach ($futureTag in @('v0.15.3', 'v1.0.0')) {
    ...
    $archivePath = Join-Path $protocolFixtureRoot 'protocol-v0.15.2.zip'
    Invoke-TestGit -Repository $protocolRepository -Arguments @(
        'archive', '--format=zip', '--prefix=openai-mock-protocol/',
        '-o', $archivePath, 'v0.15.2'
    ) | Out-Null
...
function New-MockConnectedManagedConsumer {
    ...
    $reuseKey = 'connected-managed-v0.15.2'
    $owner = Get-QuickAdoptionReusableFixture -Key $reuseKey -Builder {
        New-MockConnectedManagedConsumerPair `
            -Name 'owner-connected-managed-v0.15.2' `
            -InstalledTag 'v0.15.2'
    } -InputRecords @(
        'installed-tag=v0.15.2',
...
function New-MockCompletedAdoptionConsumer {
    ...
    $completedBranch = 'automation/meandai-capabilities-v0.15.2'
...
function Initialize-MockAdoptionPullRequestBody {
    ...
    $markerRecord = if ($null -ne $sourceGraphIdentity) {
        [ordered]@{
            ...
            branch = 'automation/meandai-capabilities-v0.15.2'
...
function New-TestQuickAdoptionPullRequestContractFixture {
    ...
    $pullRequest = [pscustomobject][ordered]@{
        ...
        headRefName = 'automation/meandai-capabilities-v0.15.2'
        headRefOid = $head
        baseRefName = 'main'
...
function Test-MockInitialPolicyAuthorityCall {
    ...
    return [string]$endpoint[0] -cin @(
        'repos/hasanmanzak/meAndAI/releases/tags/v0.15.2',
        'repos/hasanmanzak/meAndAI/commits/v0.15.2',
        'repos/hasanmanzak/meAndAI/contents/templates/project/.github/scripts/MeAndAI.CapabilitiesBootstrap.psm1?ref=v0.15.2'
    )
}
...
function global:gh {
    ...
    if ($Arguments.Count -ge 2 -and $Arguments[0] -eq 'issue' -and $Arguments[1] -eq 'create') {
        ...
        $createdIssue = [pscustomobject]@{
            ...
            title = 'Track meAndAI AI capabilities adoption from v0.15.2'
...
try {
    ...
    if (Test-QuickAdoptionShard -Name 'ContractsPreflight') {
        ...
        $assetIsolationKey = "v0.15.2`n$($canonicalInitialAdoptionPolicyAsset.TemplatePath)"
    ...
    if (Test-Path -LiteralPath $mockCodexScriptPath -PathType Leaf) {
        ...
        if (-not $mockCodex.Contains('automation/meandai-capabilities-v0.15.2')) {
    ...
    if ($failures.Count -eq 0) {
        foreach ($releaseMode in @(
            ...
            $releaseCalls = @($global:QuickAdoptionRestCalls | Where-Object {
                $_.Uri -match '/repos/hasanmanzak/meAndAI/releases/tags/v0\.15\.2$'
            })
    ...
    if ((Test-QuickAdoptionShard -Name 'AdoptionLifecycle') -and
        ...
        $initialPolicyRestCalls = @($global:QuickAdoptionRestCalls |
            Where-Object {
                [string]$_.Uri -ceq
                    'https://api.github.com/repos/hasanmanzak/meAndAI/contents/templates/project/.github/scripts/MeAndAI.CapabilitiesBootstrap.psm1?ref=v0.15.2'
            })
        $workflowRestCalls = @($global:QuickAdoptionRestCalls |
            Where-Object {
                [string]$_.Uri -ceq
                    'https://api.github.com/repos/hasanmanzak/meAndAI/contents/templates/project/.github/workflows/meandai-protocol-update.yml?ref=v0.15.2'
            })
        ...
        $canonicalIssueMarker = Get-TestCanonicalAdoptionIssueMarker `
            -Repository $global:QuickAdoptionRepoName -TargetTag 'v0.15.2' `
            -ProtocolSha $global:QuickAdoptionProtocolSha `
        ...
        $expectedIssueReferences = @(
            "- Protocol release: [v0.15.2](https://github.com/hasanmanzak/meAndAI/releases/tag/v0.15.2)",
        ...
        $adoptionPaths = @(Invoke-Git -Repository $existingRemote -Arguments @(
            'ls-tree', '-r', '--name-only', 'refs/heads/automation/meandai-capabilities-v0.15.2'
        ))
        ...
        $protocolSourceEndpoint = 'repos/hasanmanzak/meAndAI/contents/templates/project/.github/workflows/meandai-protocol-update.yml?ref=v0.15.2'
        ...
        $initialPolicySourceEndpoint = 'repos/hasanmanzak/meAndAI/contents/templates/project/.github/scripts/MeAndAI.CapabilitiesBootstrap.psm1?ref=v0.15.2'
        ...
        $interruptedRemoteHead = (@(Invoke-Git -Repository $existingRepo -Arguments @(
            'ls-remote', '--heads', 'origin',
            'refs/heads/automation/meandai-capabilities-v0.15.2'
        )))[0].Split("`t")[0]
    ...
    if (Test-QuickAdoptionShard -Name 'InstructionGraphClosure') {
        ...
        $graphAwareBranch = 'automation/meandai-capabilities-v0.15.2'
        ...
        $graphAwareMarkerValid = & $markerValidator `
            ...
            -BaseBranch 'main' -TargetTag 'v0.15.2' `
    ...
    if ($runIntegrityShards -and
        ...
        $validManifestAccepted = & $manifestValidator `
            ...
            -TargetTag 'v0.15.2' -ProtocolSha ('a' * 40) `
        ...
        if ($canonicalOwnedIssue.Count -eq 1) {
            Reset-MockAdoptionProposal
            $legacyIssueMarker =
                '<!-- meandai-local-adoption:v0.15.2:pr-42 -->'
            $pullRequestUrl =
    ...
    if ($runIntegrityShards -and
        ...
        $collisionEntry = @((Invoke-Git -Repository $existingRemote -Arguments @(
            'ls-tree', 'refs/heads/automation/meandai-capabilities-v0.15.2', '--', '.ai/protocol'
        )))
        ...
        $migrationBranchApplicationBlob = (@(Invoke-TestGit `
            -Repository $existingRemote -Arguments @(
                'rev-parse',
                'refs/heads/automation/meandai-capabilities-v0.15.2:app.txt'
            )))[0]
        $migrationAgentsContent = @(Invoke-TestGit `
            -Repository $existingRemote -Arguments @(
                'show',
                'refs/heads/automation/meandai-capabilities-v0.15.2:AGENTS.md'
            )) -join "`n"
    ...
    if ((Test-QuickAdoptionShard -Name 'RepositoryRoutes') -and
        ...
        if ($modifiedSeedError -notlike
                '*not the exact canonical v0.15.2 file*no GitHub repository or remote was changed*' -or
        ...
        $currentConsumer = New-MockConnectedManagedConsumer `
            -Name 'managed-current' -InstalledTag 'v0.15.2'
        ...
        foreach ($blockedRoute in @(
            [pscustomobject]@{ Name = 'manifest'; InstalledTag = 'v0.15.2'; TargetTag = 'v0.15.2'; Mutation = 'Manifest' },
            [pscustomobject]@{ Name = 'partial'; InstalledTag = 'v0.15.2'; TargetTag = 'v0.15.2'; Mutation = 'Partial' },
            [pscustomobject]@{ Name = 'missing-gitlink'; InstalledTag = 'v0.15.2'; TargetTag = 'v0.15.2'; Mutation = 'MissingGitlink' },
            [pscustomobject]@{ Name = 'drift'; InstalledTag = 'v0.15.2'; TargetTag = 'v0.15.2'; Mutation = 'Drift' },
            [pscustomobject]@{ Name = 'newer'; InstalledTag = 'v0.15.3'; TargetTag = 'v0.15.2'; Mutation = 'None' },
            [pscustomobject]@{ Name = 'cross-major'; InstalledTag = 'v0.15.2'; TargetTag = 'v1.0.0'; Mutation = 'None' }
        )) {
            Reset-Mocks
