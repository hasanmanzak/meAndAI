...
function Initialize-QuickAdoptionImmutableFixture {
    ...
    $currentSha = (@(Invoke-TestGit -Repository $protocolRepository `
        -Arguments @('rev-parse', 'HEAD')))[0]
    $releaseCommits['v0.11.1'] = $currentSha
    Save-MockProtocolAssetSnapshot -Tag 'v0.11.1' `
        -Repository $protocolRepository -Snapshots $assetSnapshots

    Invoke-TestGit -Repository $protocolRepository -Arguments @(
        'switch', '--detach'
    ) | Out-Null
    foreach ($futureTag in @('v0.11.2', 'v1.0.0')) {
    ...
    $archivePath = Join-Path $protocolFixtureRoot 'protocol-v0.11.1.zip'
    Invoke-TestGit -Repository $protocolRepository -Arguments @(
        'archive', '--format=zip', '--prefix=openai-mock-protocol/',
        '-o', $archivePath, 'v0.11.1'
    ) | Out-Null
...
function New-MockCompletedAdoptionConsumer {
    ...
    $completedBranch = 'automation/meandai-capabilities-v0.11.1'
...
function Publish-MockAdoptionBranch {
    $branch = 'automation/meandai-capabilities-v0.11.1'
...
function Test-MockInitialPolicyAuthorityCall {
    ...
    return [string]$endpoint[0] -cin @(
        'repos/hasanmanzak/meAndAI/releases/tags/v0.11.1',
        'repos/hasanmanzak/meAndAI/commits/v0.11.1',
        'repos/hasanmanzak/meAndAI/contents/templates/project/.github/scripts/MeAndAI.CapabilitiesBootstrap.psm1?ref=v0.11.1'
    )
}
...
function global:gh {
    ...
    if ($Arguments.Count -ge 2 -and $Arguments[0] -eq 'issue' -and $Arguments[1] -eq 'create') {
        ...
        $createdIssue = [pscustomobject]@{
            ...
            title = 'Track meAndAI AI capabilities adoption from v0.11.1'
    ...
    if ($Arguments.Count -ge 2 -and $Arguments[0] -eq 'pr' -and $Arguments[1] -eq 'list') {
        ...
        $pullRequest = [ordered]@{
            ...
            headRefName = 'automation/meandai-capabilities-v0.11.1'
...
try {
    ...
    if (Test-Path -LiteralPath $mockCodexScriptPath -PathType Leaf) {
        ...
        if (-not $mockCodex.Contains('automation/meandai-capabilities-v0.11.1')) {
    ...
    if ($failures.Count -eq 0) {
        foreach ($releaseMode in @('Mutable', 'Missing')) {
            ...
            $releaseCalls = @($global:QuickAdoptionRestCalls | Where-Object {
                $_.Uri -match '/repos/hasanmanzak/meAndAI/releases/tags/v0\.11\.1$'
            })
    ...
    if ((Test-QuickAdoptionShard -Name 'AdoptionLifecycle') -and
        ...
        $initialPolicyRestCalls = @($global:QuickAdoptionRestCalls |
            Where-Object {
                [string]$_.Uri -ceq
                    'https://api.github.com/repos/hasanmanzak/meAndAI/contents/templates/project/.github/scripts/MeAndAI.CapabilitiesBootstrap.psm1?ref=v0.11.1'
            })
        $workflowRestCalls = @($global:QuickAdoptionRestCalls |
            Where-Object {
                [string]$_.Uri -ceq
                    'https://api.github.com/repos/hasanmanzak/meAndAI/contents/templates/project/.github/workflows/meandai-protocol-update.yml?ref=v0.11.1'
            })
        ...
        $canonicalIssueMarker = '<!-- meandai-local-adoption:v0.11.1:pr-42 -->'
        ...
        $adoptionPaths = @(Invoke-Git -Repository $existingRemote -Arguments @(
            'ls-tree', '-r', '--name-only', 'refs/heads/automation/meandai-capabilities-v0.11.1'
        ))
        ...
        $protocolSourceEndpoint = 'repos/hasanmanzak/meAndAI/contents/templates/project/.github/workflows/meandai-protocol-update.yml?ref=v0.11.1'
        ...
        $initialPolicySourceEndpoint = 'repos/hasanmanzak/meAndAI/contents/templates/project/.github/scripts/MeAndAI.CapabilitiesBootstrap.psm1?ref=v0.11.1'
        ...
        $interruptedRemoteHead = (@(Invoke-Git -Repository $existingRepo -Arguments @(
            'ls-remote', '--heads', 'origin',
            'refs/heads/automation/meandai-capabilities-v0.11.1'
        )))[0].Split("`t")[0]
        ...
        if ($canonicalOwnedIssue.Count -ne 1) {
            ...
            $markerVariants = [ordered]@{
                malformed = $canonicalOwnedBody.Replace(
                    ...
                    '<!-- meandai-local-adoption:v0.11.1:pr-42 --'
                )
    ...
    if ($runIntegrityShards -and
        ...
        $collisionEntry = @((Invoke-Git -Repository $existingRemote -Arguments @(
            'ls-tree', 'refs/heads/automation/meandai-capabilities-v0.11.1', '--', '.ai/protocol'
        )))
        ...
        $migrationBranchApplicationBlob = (@(Invoke-TestGit `
            -Repository $existingRemote -Arguments @(
                'rev-parse',
                'refs/heads/automation/meandai-capabilities-v0.11.1:app.txt'
            )))[0]
    ...
    if ((Test-QuickAdoptionShard -Name 'RepositoryRoutes') -and
        ...
        if ($modifiedSeedError -notlike
                '*not the exact canonical v0.11.1 file*no GitHub repository or remote was changed*' -or
        ...
        $currentConsumer = New-MockConnectedManagedConsumer `
            -Name 'managed-current' -InstalledTag 'v0.11.1'
        ...
        foreach ($blockedRoute in @(
            [pscustomobject]@{ Name = 'manifest'; InstalledTag = 'v0.11.1'; TargetTag = 'v0.11.1'; Mutation = 'Manifest' },
            [pscustomobject]@{ Name = 'partial'; InstalledTag = 'v0.11.1'; TargetTag = 'v0.11.1'; Mutation = 'Partial' },
            [pscustomobject]@{ Name = 'missing-gitlink'; InstalledTag = 'v0.11.1'; TargetTag = 'v0.11.1'; Mutation = 'MissingGitlink' },
            [pscustomobject]@{ Name = 'drift'; InstalledTag = 'v0.11.1'; TargetTag = 'v0.11.1'; Mutation = 'Drift' },
            [pscustomobject]@{ Name = 'newer'; InstalledTag = 'v0.11.2'; TargetTag = 'v0.11.1'; Mutation = 'None' },
            [pscustomobject]@{ Name = 'cross-major'; InstalledTag = 'v0.11.1'; TargetTag = 'v1.0.0'; Mutation = 'None' }
        )) {
            Reset-Mocks
...
'@
                    ...
                    function Invoke-External {
                        ...
                        if ($Arguments[2] -ceq 'hasanmanzak/meAndAI') {
                            ...
                            if ($LASTEXITCODE -ne 0 -or
                                $clonedTags -cnotcontains 'v0.11.1') {
            ...
            if ($record.CurrentLauncher -isnot [bool] -or
                ...
                [string]$record.TargetTag -cne 'v0.11.1' -or
