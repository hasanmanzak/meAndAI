...
| Tests | Existing [TEST-0069](../FEAT-0012-v082-correction/test-cases.md#test-0069), related [TEST-0176](../FEAT-0047-v0142-clickable-cross-record-references/test-cases.md#test-0176), and retained hosted-streaming [TEST-0105](../FEAT-0020-v095-streamed-codex-cancellation/test-cases.md#test-0105); [feature evidence](test-cases.md) |
...
| `FIND-0360` <a name="find-0360"></a> | Hosted streaming evidence / P1 | Two independent Windows PowerShell 5.1 hosted runs processed the first mock JSONL event but stopped because the child did not observe the test-only file acknowledgment within its five-second window. The bidirectional filesystem handshake could therefore fail a required gate without disproving incremental production consumption. | `Blocking` / Resolved before merge in exact correction commit [`120b04f6d724a1c01971d59332f3c54b2ada789c`](https://github.com/hasanmanzak/meAndAI/commit/120b04f6d724a1c01971d59332f3c54b2ada789c) by removing the acknowledgment file from canonical [TEST-0105](../FEAT-0020-v095-streamed-codex-cancellation/test-cases.md#test-0105). The first replacement used PID plus process-start ticks; [FIND-0361](#find-0361) records and resolves its separate cross-platform portability defect. No production source changed, and the exact final hosted head remains the merge gate. |
| `FIND-0361` <a name="find-0361"></a> | Cross-platform live-process identity / P1 | Exact evidence head [`aa8c9f479fe902972fcd4b103e0e2353dc6ba5e0`](https://github.com/hasanmanzak/meAndAI/commit/aa8c9f479fe902972fcd4b103e0e2353dc6ba5e0) failed only the aggregate [TEST-0105](../FEAT-0020-v095-streamed-codex-cancellation/test-cases.md#test-0105) live-consumption assertion on Ubuntu in [run `30278228996`](https://github.com/hasanmanzak/meAndAI/actions/runs/30278228996); no separate required-event or safe-presentation assertion reported a failure. The hosted log did not expose which aggregate operand failed. Code-path diagnosis identified exact process-start-tick equality as the remaining platform-sensitive run-correlation discriminator and therefore an unsuitable portable oracle. | `Blocking` / Resolved before merge in exact correction commit [`345647fbd2affaba787b3b1db3a8c295a9eae49d`](https://github.com/hasanmanzak/meAndAI/commit/345647fbd2affaba787b3b1db3a8c295a9eae49d) by passing one parent-generated lowercase 32-hex run identity to the fixture and requiring that exact identity plus its bounded live PID when the first event is consumed. Exit zero, every expected event, final-result authority, safe presentation, and cancellation coverage remain mandatory. The focused owner passes on PowerShell 7 / Windows PowerShell 5.1 in 10.1 / 12.8 seconds; exact final hosted confirmation remains required. |
...
| Focused PowerShell 7 / Windows PowerShell 5.1 | The final frozen implementation bytes passed `IntegrityManifestIssue` on PowerShell 7 / Windows PowerShell 5.1 in 141.9 / 147.7 seconds. The runs include exact protocol-read token forwarding through the full launcher, all six classifications, 20 supported immutable identities, exact candidate/inventory ceilings, provider-call contracts, frozen drift, full-state zero-mutation negatives, exact manifest-root and open-PR-list shape rejection, and idempotent convergence. |
...
| Final-head streaming recurrence | [Lifecycle/backlog run `30244639416`](https://github.com/hasanmanzak/meAndAI/actions/runs/30244639416) and evidence-head [run `30274245841`](https://github.com/hasanmanzak/meAndAI/actions/runs/30274245841) independently failed only the Windows PowerShell 5.1 [TEST-0105](../FEAT-0020-v095-streamed-codex-cancellation/test-cases.md#test-0105) file-observation handshake after rendering `Codex | Session started`; Ubuntu remained green. Exact commit [`120b04f6d724a1c01971d59332f3c54b2ada789c`](https://github.com/hasanmanzak/meAndAI/commit/120b04f6d724a1c01971d59332f3c54b2ada789c) removed that handshake and passed the focused owner in 9.7 / 11.3 seconds. Exact evidence head [`aa8c9f479fe902972fcd4b103e0e2353dc6ba5e0`](https://github.com/hasanmanzak/meAndAI/commit/aa8c9f479fe902972fcd4b103e0e2353dc6ba5e0) then failed only the aggregate live-consumption assertion on Ubuntu [run `30278228996`](https://github.com/hasanmanzak/meAndAI/actions/runs/30278228996); no separate event-presence or safe-presentation assertion failed, while code-path diagnosis identified exact process-start-tick equality as the remaining platform-sensitive correlation discriminator. Final correction [`345647fbd2affaba787b3b1db3a8c295a9eae49d`](https://github.com/hasanmanzak/meAndAI/commit/345647fbd2affaba787b3b1db3a8c295a9eae49d) uses a parent-generated run identity plus bounded live PID and passes PowerShell 7 / Windows PowerShell 5.1 in 10.1 / 12.8 seconds. Final protected hosted confirmation remains required. |
