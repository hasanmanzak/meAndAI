...
| `FIND-0360` <a name="find-0360"></a> | Hosted streaming evidence / P1 | Two independent Windows PowerShell 5.1 hosted runs processed the first mock JSONL event but stopped because the child did not observe the test-only file acknowledgment within its five-second window. The bidirectional filesystem handshake could therefore fail a required gate without disproving incremental production consumption. | `Blocking` / Resolved before merge in exact correction commit [`120b04f6d724a1c01971d59332f3c54b2ada789c`](https://github.com/hasanmanzak/meAndAI/commit/120b04f6d724a1c01971d59332f3c54b2ada789c) by making canonical [TEST-0105](../FEAT-0020-v095-streamed-codex-cancellation/test-cases.md#test-0105) emit the fixture PID and process-start ticks, retain the five-second live window, and require that exact process identity to be alive when the first event is consumed. No production source changed; focused PowerShell 7 / Windows PowerShell 5.1 runs pass, and the exact final hosted head remains the merge gate. |
...
| Final-head streaming recurrence | [Lifecycle/backlog run `30244639416`](https://github.com/hasanmanzak/meAndAI/actions/runs/30244639416) and final evidence-head [run `30274245841`](https://github.com/hasanmanzak/meAndAI/actions/runs/30274245841) independently failed only the Windows PowerShell 5.1 [TEST-0105](../FEAT-0020-v095-streamed-codex-cancellation/test-cases.md#test-0105) file-observation handshake after rendering `Codex | Session started`; Ubuntu remained green. Exact correction commit [`120b04f6d724a1c01971d59332f3c54b2ada789c`](https://github.com/hasanmanzak/meAndAI/commit/120b04f6d724a1c01971d59332f3c54b2ada789c) replaces that test-only handshake with PID plus process-start identity and passes the focused owner on PowerShell 7 / Windows PowerShell 5.1 in 9.7 / 11.3 seconds. Final protected hosted confirmation remains required. |
