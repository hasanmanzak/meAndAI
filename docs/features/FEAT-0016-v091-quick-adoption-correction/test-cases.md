...
| 2026-07-16 | BUG-0005 first complete-suite working tree | Windows PowerShell 5.1 outside the restricted Git signal-pipe sandbox | `powershell -NoProfile -ExecutionPolicy Bypass -File tests/protocol.tests.ps1` | Failed after unaffected child suites passed: three regex-escaped v0.9.0 mock matchers caused quick-adoption REST calls to escape to the real API (`FIND-0146`) |
| 2026-07-16 | BUG-0005 complete-suite confirmation | Windows PowerShell 5.1 outside the restricted Git signal-pipe sandbox | `powershell -NoProfile -ExecutionPolicy Bypass -File tests/protocol.tests.ps1` | Pass in 473.8 seconds: all discovered suites; quick-adoption machine-readable evidence includes `TEST-0100` |

Hosted and post-publication facts remain external in issue #49.
