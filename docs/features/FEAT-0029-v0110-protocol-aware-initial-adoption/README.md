...
| `FIND-0160` <a name="find-0160"></a> | Windows `Full`-profile timeout under-provisioning / `Blocking` then resolved locally | `p1` / the correct serial Windows profile passed the capabilities and updater families but GitHub canceled the still-running suite at the stale 20-minute job limit | At that historical correction only the Windows job bound became 35 minutes, giving the measured 1576-second local `Full` run about 33 percent headroom; Linux was then 20 minutes and post-publication 5, with no added job, matrix, setup, or coverage route. [TEST-0124](../FEAT-0027-v0104-runner-minute-efficiency/test-cases.md#test-0124) failed first against 20 and passed against that exact historical Windows boundary. Later current bounds are owned by the canonical runner-efficiency records; this row is historical and does not override them. |
