...
| `TEST-0053` | `SUBF-0021` | Rename or case-move the protected updater workflow or a credential path while leaving only an allowed-looking destination. | Both source and destination are evaluated ordinally and publication blocks. | Path policy / real Git | Passed | Staged name-status fixture |
...
| 2026-07-15 | PR #35 working tree after `FIND-0092` | Windows PowerShell 5.1 | `tests/quick-adoption.tests.ps1`; `tests/protocol.tests.ps1 -StructureOnly` | Pass through `TEST-0059` in 119 seconds; hosted rerun pending |
