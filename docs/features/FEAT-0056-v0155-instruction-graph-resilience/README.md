...
`must read/load/consult` forms from descriptive `load`/`consult` text. Existing
...
| `RISK-0258` <a name="risk-0258"></a> | Narrowing `load`/`consult` loses a real imperative required-reading declaration. | Parser owner / retain anchored `Read/Load/Consult` plus explicit `must read/load/consult`, required headings/lists, roots, negation, and authority/index matrices. |
...
3. Markdown links/reference definitions, tracked repeated-hash paths containing
   ...
   `load`/`consult` text is not required reading while explicit imperative and
   `must read/load/consult` forms remain required. An absent all-numeric slash
...
- [x] One canonical full suite passes on the final candidate tree.
