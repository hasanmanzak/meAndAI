...
| --- | --- | --- |
| Scenarios | [TEST-0220](test-cases.md#test-0220) and [TEST-0221](test-cases.md#test-0221) complete; [TEST-0210](test-cases.md#test-0210) remains `Planned` | Sixteen of twenty live A packets are `ReviewedLocalGreen` (`80%`). A-PREDECESSOR is exact-head hosted-green at correction graph `351 nodes / 4096 edges`, with Windows `35m20s`, Ubuntu `19m00s`, and publication verification correctly skipped; A-COMPLETE remains immutable hosted predecessor history. Never-activated `A-CONVERGE-01` remains retired/excluded; `A-TRANSITION-01` and later remain Candidate/inactive. Partial Facts retain only `ContractSlice=A`; cumulative A is `29/29`. |
| Test code | [TEST-0210](test-cases.md#test-0210) remains `Planned`; packet-local cumulative A is `29/29` | A-PREDECESSOR focused, retained A-COMPLETE, cumulative A, full Conformance, and full Domain are exact `1/1`, `1/1`, `29/29`, `29/29`, and `98/98`. Release build/format/diff/locks and StructureOnly are green; three independent reviews are `0/0/0`. Realized A-PREDECESSOR production/test/combined gross deltas are `120/416/536`. Full [TEST-0210](test-cases.md#test-0210) green and A completion are not claimed. |
| Current immutable delivery | Exact A-PREDECESSOR correction head plus retained A-COMPLETE predecessor history | [FIND-0460](#find-0460) resolves the bounded graph-only exposure and records A-PREDECESSOR exact-head hosted-green at Windows `35m20s` and Ubuntu `19m00s`; publication verification was correctly skipped. Canonical R `0014` remains immutable and was not rerun. `A-TRANSITION-01`, publication, and all later scope remain held until this records-only delivery head independently becomes exact-hosted-green. |
...
graph is `4095/4096`. `A-PREDECESSOR-01` is now exact-head hosted-green
`ReviewedLocalGreen` at the immutable correction identity in the canonical
owning finding; its graph is `351 nodes / 4096 edges`, Windows passed in `35m20s`, Ubuntu in
`19m00s`, publication verification was correctly skipped, cumulative A/full
Conformance are `29/29`, progress is `16/20` (`80%`), and `A-TRANSITION-01`
remains Candidate/inactive until this records-only delivery head is itself
exact-hosted-green.
