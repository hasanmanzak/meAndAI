...
| Status | [SUBF-0152](#subf-0152) and [SUBF-0153](#subf-0153) complete; [SUBF-0143](#subf-0143) Gate 2 accepted. Exact admission and projector frozen-design predecessor evidence remain unchanged. Thirteen of twenty live packets are `ReviewedLocalGreen` (`65%`), cumulative A is `26/26`, and [TEST-0210](test-cases.md#test-0210) stays `Planned`. `A-PROJECTOR-DAG-01` is packet-local `ReviewedLocalGreen`; [FIND-0451](#find-0451) is resolved. [FIND-0452](#find-0452) corrects the A-FULL sibling-test allowlist before R. Never-activated `A-CONVERGE-01` is retired/excluded; one-for-one replacement `A-FULL-MANIFEST-01` is `FrozenDesign / RenewedSiblingContractReviewed / PendingRecordDelivery` and inactive. Later packets remain Candidate/inactive. Final Scenario/status/owner/workflow/[TEST-0146](../FEAT-0035-test-runtime-efficiency/test-cases.md#test-0146), B/C/D, merge, release, and publication remain held; no completion/DoD is claimed. |
| Hosted correction | Historical heads and [FIND-0445](#find-0445) through [FIND-0452](#find-0452) retain or await their exact evidence. The records-only graph-budget correction is exact-head hosted-green; the bounded sibling-contract record correction passed renewed review `0/0/0` and awaits records-only commit/push plus hosted proof. Publication remains held. |
...
| `FIND-0452` <a name="find-0452"></a> | Pre-R implementation-readiness review proved that the frozen generic admission rule and the two-file green allowlist could not both hold: generic production permits distinct canonical contract keys across Observed, Failed, and NoInput, while the existing admission sibling still rejected changing only the Observed key. The sibling's mixed-version rejection is independent and remains valid. No R or production mutation existed when found. | Classification `VerifiedDefect`; disposition `CorrectionPrepared`; severity `medium`; confidence `high`; dependencies `None`; priority `p0`; impact rank `high`; status `InProgress`. Green will require exactly one common contract version but no contract-key cardinality, and will convert only the changed-key rejection into a positive parse/write byte round-trip. The sibling stays unchanged through P/R. Revised caps are production `80`, new test retained source `608`, sibling gross delta `12`, aggregate tests `620`, combined `700`. R marker/FQN/ordinal remain unchanged. Renewed review closed `0/0/0`; records-only commit/push and exact-head hosted proof remain pending. |
...
`FrozenDesign / RenewedSiblingContractReviewed / PendingRecordDelivery`: its graph-budget
correction is hosted-green, while the bounded admission sibling allowlist
correction passed renewed review `0/0/0` and now awaits records-only commit/push
plus exact-head hosted validation before R. Later packets remain
Candidate/inactive.
