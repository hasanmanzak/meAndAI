...
Applicability and Evaluation cohorts are exact-head hosted green; Intent is a
separate packet-local `ReviewedLocalGreen` result in the unpushed Results/
closure cohort. C is `10/11`, and current A+B+C is `53/53`. Applicability-plan R=0006 is
...
and Domain `98/98`; `C-INTENT-RESULT-01` is `ReviewedLocalGreen`, while
`C-AGGREGATION-01` remains next/`FrozenDesign` until that focused commit exists.
