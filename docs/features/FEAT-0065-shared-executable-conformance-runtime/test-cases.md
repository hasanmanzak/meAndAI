...
Applicability cohorts are exact-head hosted green; Evaluation Plan is packet-
local `ReviewedLocalGreen` in an unpushed cohort, C is `8/11`, and current
A+B+C is `51/51`. Applicability-plan R=0006 is
...
green. Evaluation Plan is packet-local `ReviewedLocalGreen` in an unpushed
cohort; Evaluation Advance is next/`FrozenDesign`, and every later scope remains
held.
