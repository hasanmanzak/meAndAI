...
`A-TARGET-PARSER-INDEX-SLOT-01` is a `FrozenDesign`,
...
`A-GOVERNED-REFERENCE-SLOTS-01` is `ReviewedLocalGreen`; target implementation
remains inactive under its frozen design, every later packet remains
Candidate/inactive, and no workflow, status, owner,
...
packet had not yet acquired an exact-head hosted claim. Current target-freeze
