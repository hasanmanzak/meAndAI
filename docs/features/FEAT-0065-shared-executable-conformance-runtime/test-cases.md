...
| Expected-red purity | One ContractSlice at a time after byte-identical locked restore: SurfaceRed is compile-only with exactly the design's one `CS0246` file/span/token/FQN tuple and no discovery claim. A's first BehaviorRed used the historical `ParseCanonical => null!` predicate. The second uses `ContractSliceACanonicalStringTests.Enforces_exact_canonical_manifest_string_encoding` / `TEST-0210-A-BEHAVIOR-RED-0002` and only the frozen legacy quoted-byte result. The third uses `ContractSliceASchemaSlotManifestTests.Enforces_exact_schema_and_zero_capability_evidence_slot_closure` / `TEST-0210-A-BEHAVIOR-RED-0003`. The fourth uses `ContractSliceAIndexSlotManifestTests.Enforces_exact_repository_tree_index_and_slot_capability_closure` / `TEST-0210-A-BEHAVIOR-RED-0004`. The fifth uses exact FQN `MeAndAI.Protocol.Conformance.Tests.ContractSliceAParserRecordSlotManifestTests.Enforces_exact_markdown_parser_protocol_record_index_and_slot_capability_closure` / `TEST-0210-A-BEHAVIOR-RED-0005`. The sixth uses exact FQN `MeAndAI.Protocol.Conformance.Tests.ContractSliceAGovernedReferenceSlotsManifestTests.Enforces_exact_governed_reference_index_and_dual_governed_text_slot_capability_closure` / `TEST-0210-A-BEHAVIOR-RED-0006`. The seventh uses exact FQN `MeAndAI.Protocol.Conformance.Tests.ContractSliceATargetParserIndexSlotManifestTests.Enforces_exact_repository_target_schema_parser_index_and_slot_capability_closure` / `TEST-0210-A-BEHAVIOR-RED-0007`, with only `ContractSlice=A` and no `Scenario`. For 0003, 0004, and 0005 only `InvalidOperationException` with exact legacy writer message `This writer increment supports only the minimal qualification slice.` may invoke `Assert.Fail`; 0005 runs the writer before direct factory/matrix assertions. For 0006 and 0007, all setup, successful Catalog creation, the validation-free parsed manifest, and the runtime-created expected exception remain outside the catch; only the internal production writer boundary is guarded, and only exact runtime `ArgumentException`, `ParamName=rules`, and exact runtime-built parser-graph Message may invoke `Assert.Fail`. B/C/D retain their exact warning-free activation/export `null!` sentinel. Every BehaviorRed uses one full-FQN-filtered invocation, one TRX logger, a fresh external TRX path, exactly one selected/executed/failed result, and the complete zero/nonzero counter inventory. The sole failed result has exactly one `Output/ErrorInfo/Message` equal to the marker byte-for-byte; `ErrorInfo/StackTrace` is absent or one nonempty marker-free standard assertion stack, and no other `ErrorInfo` child is allowed. At most one byte-identical summary echo and one exact marker-free same-FQN `[FAIL]` RunInfo are accepted same-result bookkeeping; all sixteen counters including `error=0` and absence of other diagnostics/attachments remain exact. Reviewed source plus immutable xUnit `2.9.3` lock proves the direct `Assert.Fail` exception type because standard TRX does not serialize it. C first proves executable export activation; D first proves only the real export graph. RULE-0001 waits for the real D writer/codec/parser/index/projector/kernel vector. Restore/lock/project/analyzer/predecessor/environment defects, independent or contradictory diagnostics/results, relabeled unexpected exceptions, or console-only inference invalidate red. |
...
[TEST-0210](#test-0210) stays `Planned`; twelve of twenty live packets are
`ReviewedLocalGreen` (`60%`), never-activated `A-PARSER-INDEX-01` is retired,
...
`A-GOVERNED-REFERENCE-SLOTS-01`, `A-FINDING-01`, `A-SELECTOR-01`, and
`A-ADMISSION-01` are `ReviewedLocalGreen`; cumulative A is `25/25`. The exact
...
and remains the design predecessor. Exact hosted-green admission implementation
delivery [`c1653d45c99eb01291bc571e93d74db80d94d9e8`](https://github.com/hasanmanzak/meAndAI/commit/c1653d45c99eb01291bc571e93d74db80d94d9e8),
...
publication verification was correctly skipped. Its synchronized record-evidence
commit/push and exact-head hosted validation remain pending. Every later packet
remains Candidate/inactive, and no workflow,
status, owner,
...
`ReviewedLocalGreen`; cumulative A is `25/25`. Exact hosted-green admission
...
`18m12s` and Windows in `17m28s` in
[run 30778711538](https://github.com/hasanmanzak/meAndAI/actions/runs/30778711538),
with publication verification correctly skipped. Its record-evidence
commit/push and exact-head hosted validation remain pending; later packets, final
...
`A-SELECTOR-01` is exact-head `ReviewedLocalGreen`; `A-ADMISSION-01` is
packet-local `ReviewedLocalGreen` at `12/20`, `60%`, and cumulative A `25/25`.
Its exact hosted-green implementation delivery is
...
Its record-evidence commit/push and exact-head hosted validation remain pending;
...
`A-ADMISSION-01` is packet-local `ReviewedLocalGreen`; twelve of twenty live
packets are green (`60%`) and cumulative A is `25/25`. Exact hosted-green
...
[run 30778711538](https://github.com/hasanmanzak/meAndAI/actions/runs/30778711538);
publication verification was correctly skipped. Its synchronized record-evidence
commit/push and exact-head hosted validation remain pending.
Every later packet remains Candidate/inactive, and [TEST-0210](#test-0210)
remains `Planned`; final Scenario/status/owner/workflow/[TEST-0146](../FEAT-0035-test-runtime-efficiency/test-cases.md#test-0146)
remain held.
