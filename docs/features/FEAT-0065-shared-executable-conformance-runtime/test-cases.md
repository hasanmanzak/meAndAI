...
green. Evaluation Plan and Advance are separate packet-local
`ReviewedLocalGreen` commits in one unpushed cohort; corrected Advance R=0015
is accepted/immutable, its green is focused `1/1`, C `9/9`, full Conformance
`52/52`, and Domain `98/98`, and every later scope remains held pending the
Evaluation cohort full local gate, single push, and exact-head hosted green.
