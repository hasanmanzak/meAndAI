...
`ReviewedLocalGreen` at B `9/11`, cumulative A+B `41/43`, with its implementation
head hosted validation pending. Later packets remain held.
...
R=0011 is accepted and `B-ADMISSION-01` is `ReviewedLocalGreen`, with its
implementation head hosted validation pending. Its executable allowlist remains
...
retained validation gates. R=0008, R=0009, and R=0010 are never rerun. Corrected R=0011 and
...
B-ADMISSION is therefore `ReviewedLocalGreen`; exact-head hosted validation is
the remaining packet gate.
