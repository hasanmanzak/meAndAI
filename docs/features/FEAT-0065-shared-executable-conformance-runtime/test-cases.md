...
`ReviewedLocalGreen`; cumulative A is `24/24`. Selector green delivery is
...
`A-FINDING-01` and `A-SELECTOR-01` are `ReviewedLocalGreen`; selector green
delivery remains pending. `A-ADMISSION-01` and later packets, final Scenario/status/owner/
...
`A-SELECTOR-01` is `ReviewedLocalGreen`; synchronized green delivery remains
pending. `A-ADMISSION-01` and every later packet remain Candidate/inactive;
...
`ReviewedLocalGreen`; synchronized commit/push/exact-head hosted validation is
