...
B-CACHE is `FrozenDesign`/inactive pending its exact design-head hosted gate and
later packets remain held.
...
`B-CACHE-01` is `FrozenDesign`/inactive. It may add only
...
runner. Required green is `1/1`, `8/8`, `40/40`, and `98/98`; B-ADMISSION and
every later packet remain held through cache implementation-head hosted green.
