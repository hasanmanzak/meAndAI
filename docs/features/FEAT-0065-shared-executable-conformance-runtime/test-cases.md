...
`B-WIRE-REPOSITORY-TARGET-01` R=0004 is immutable diagnostic-only; corrected
R=0005 is FrozenDesign/inactive pending its own exact correction-design-head hosted gate; every later
packet remains held.
...
No target-index capability semantics, resource ledger, runner, red, or C#
implementation is active before the synchronized design commit is exact-head
hosted green. Intended green is focused/B/full Conformance/Domain `1/1`, `6/6`,
`38/38`, and `98/98`; [TEST-0210](#test-0210) remains `Planned`.
