...
`ReviewedLocalGreen` (`35%`),
never-activated `A-PARSER-INDEX-01` is retired,
`A-PARSER-RECORD-SLOT-01` is packet-local `ReviewedLocalGreen /
PendingExactHeadHostedVerification` with cumulative A `20/20`, both redraw
successors remain Candidate/inactive, and no workflow, status, owner,
...
packet-local `ReviewedLocalGreen / PendingExactHeadHostedVerification`; both
later replacements remain Candidate with no FQN, marker, or ordinal.
