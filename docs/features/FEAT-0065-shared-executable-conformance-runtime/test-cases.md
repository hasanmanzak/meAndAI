...
`A-GOVERNED-REFERENCE-SLOTS-01` is `ReviewedLocalGreen`; `A-FINDING-01` is
`FrozenDesign`; `A-SELECTOR-01` and every later packet remain Candidate/inactive,
...
`A-FINDING-01` is `FrozenDesign`; `A-SELECTOR-01` and later packets, final Scenario/status/owner/
