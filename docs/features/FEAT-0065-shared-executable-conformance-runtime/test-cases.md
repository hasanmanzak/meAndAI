...
Applicability cohorts are exact-head hosted green; Evaluation Plan and Advance
are separate packet-local `ReviewedLocalGreen` commits in one unpushed cohort,
C is `9/11`, and current A+B+C is `52/52`. Applicability-plan R=0006 is
...
R=0016 is immutable diagnostic/no-success. Corrected `C-INTENT-RESULT-01` is
`FrozenDesign`/inactive with R=0017 held pending its corrected records/design
exact-head hosted gate, and every later scope remains held.
