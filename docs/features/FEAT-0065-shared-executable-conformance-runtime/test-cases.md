...
B-CONVERGE is merged/exact-main green. The C design cohort is exact-head hosted
green; C surface and registration-mismatch are `ReviewedLocalGreen`, C is
`4/11`, current A+B+C is `47/47`, and producer-pipeline is next without an
intermediate push/hosted claim; later scopes remain held.
