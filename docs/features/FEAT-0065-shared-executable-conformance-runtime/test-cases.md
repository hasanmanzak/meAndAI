...
B-CONVERGE is merged/exact-main green. The C design and Activation cohorts are
exact-head hosted green; both Applicability packets are `ReviewedLocalGreen`,
C is `7/11`, and current A+B+C is `50/50`. Applicability-plan R=0006 is
...
The Applicability cohort awaits one full local boundary, one push, and exact-
head hosted green; Evaluation and every later scope remain held.
