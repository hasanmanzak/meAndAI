...
Applicability and Evaluation cohorts are exact-head hosted green; Intent and
Aggregation are separate packet-local `ReviewedLocalGreen` results in the
unpushed Results/closure cohort. C is `11/11`, and current A+B+C/full
...
`C-INTENT-RESULT-01` and `C-AGGREGATION-01` are separate unpushed
`ReviewedLocalGreen` commits; `C-CONVERGE-01` is the next code-free audit.
Every later scope remains held.
