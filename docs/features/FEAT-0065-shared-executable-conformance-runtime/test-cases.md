...
| --- | --- |
| A - Catalog and activation | Exact cumulative-A `48` exports and members; zero Domain delta; qualification slice versus complete snapshot; canonical manifest; acyclic artifact binding; exact normative selectors/bytes/digests; predecessor transitions; schema/demand DAG; named profiles; negative registration surface; `27` Policy registration/type-contract and `35` full component rows; fail-closed missing/extra/duplicate/retired/unmapped inventory. |
...
| Expected-red purity | One ContractSlice at a time after byte-identical locked restore: SurfaceRed is compile-only with exactly the design's one `CS0246` file/span/token/FQN tuple and no discovery claim; after surface compile green, the uncommitted structural checkpoint uses only the design's exact warning-free `null!` activation/export sentinel and absent-behavior predicate; BehaviorRed uses one full-FQN-filtered invocation, a fresh external TRX path, exactly one selected/executed/failed result, the complete zero/nonzero counter inventory, and the exact reviewed assertion-type/message marker placement. D first proves only the real export graph; RULE-0001 waits until the real D writer/codec/parser/index/projector/kernel vector path is green, then uses that D session to mint a fresh Conformance-registered input from immutable fixture data. Restore/lock/project/analyzer/predecessor/environment, extra diagnostics/results, relabeled unexpected exceptions, or console-only inference are invalid red. |
