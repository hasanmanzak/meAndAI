...
| 2026-07-24 | `v0.14.2` working tree | PowerShell 7 | `pwsh -NoProfile -File tests/capabilities/protocol-governance/protocol-governance.tests.ps1` | Parser, link, and [TEST-0175](test-cases.md) fixtures produced no failure; the owner remained intentionally red only because the current feature record stays `In progress` until its exact pushed commit and delivery pull request exist |
