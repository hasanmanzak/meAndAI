...
| `FIND-0239` <a name="find-0239"></a> | The next clean hosted checkout exposed four valid historical pull-request-head commit permalinks whose objects were no longer reachable through a fetched branch or tag, while the long-lived development clone still retained them | `Blocking`, resolved: [TEST-0178](test-cases.md#test-0178) retains exact local commit-object proof when available, rejects a locally present non-commit object, defers a completely absent object in structural mode, and requires bounded owning-repository GitHub API proof in authoritative mode |
