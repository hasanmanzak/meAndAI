...
| 2026-07-19 | `FIND-0170` corrected working tree | Windows PowerShell 5.1 | Focused `capability-catalog.tests.ps1`; `tests/protocol.tests.ps1 -StructureOnly`; parse every PowerShell source; `git diff --check` | Passed in 1.7 and 4.0 seconds plus clean parse/whitespace checks; explicit UTC literals, strongly typed output, and universal adjustment preserve all positive and negative ledger cases; replacement Ubuntu evidence pending |
