...
For a new submodule consumer on `v0.13.0`, the only repository file required
before the lifecycle runs is the exact canonical
[AI capabilities lifecycle workflow](https://github.com/hasanmanzak/meAndAI/blob/v0.13.0/templates/project/.github/workflows/meandai-protocol-update.yml)
...
before checkout; the current example then installs `v0.13.0`:

```powershell
git -C .ai/protocol fetch --tags
git -C .ai/protocol checkout v0.13.0
git add .ai/protocol
```
