...
For a new submodule consumer on `v0.14.5`, the only repository file required
...
before checkout; the current example then installs `v0.14.5`:

```powershell
git -C .ai/protocol fetch --tags
git -C .ai/protocol checkout v0.14.5
git add .ai/protocol
```
