...
For a new submodule consumer on `v0.16.0`, the only repository file required
...
before checkout; the current example then installs `v0.16.0`:

```powershell
git -C .ai/protocol fetch --tags
git -C .ai/protocol checkout v0.16.0
git add .ai/protocol
```
