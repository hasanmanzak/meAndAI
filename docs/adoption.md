...
For a new submodule consumer on `v0.14.3`, the only repository file required
...
before checkout; the current example then installs `v0.14.3`:

```powershell
git -C .ai/protocol fetch --tags
git -C .ai/protocol checkout v0.14.3
git add .ai/protocol
```
