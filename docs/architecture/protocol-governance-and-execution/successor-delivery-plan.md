...
| Current authority | ContractSlice A is merged/exact-main green. B surface, codec, three wires, and B-RESOURCE are exact-head hosted green; R=0006 is accepted/immutable. B is `7/11`, A+B `39/43`; B-CACHE is `FrozenDesign`/inactive pending this exact design head's hosted gate. Later B, C/D, activation, merge/release/publication remain held. |
