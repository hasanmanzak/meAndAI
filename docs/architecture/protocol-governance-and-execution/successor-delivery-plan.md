...
| Current authority | ContractSlice A and B are merged/exact-main green, with B `11/11` and A+B `43/43`. ContractSlice C is `FrozenDesign`/inactive at `0/11`, pending its synchronized design head's exact hosted gate. D, activation, merge/release/publication remain held. |
