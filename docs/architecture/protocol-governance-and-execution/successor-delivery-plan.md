...
| Current authority | ContractSlice A is merged/exact-main green. B surface, codec activation, and repository-tree wire are exact-head hosted green; `B-WIRE-GOVERNED-TEXT-01` is `FrozenDesign`/inactive pending its synchronized design head's hosted gate at B `4/11`, A+B `36/43`. Later B packets remain predecessor-gated; C/D, activation, merge/release/publication remain held. |
