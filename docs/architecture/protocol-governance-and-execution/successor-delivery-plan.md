...
| Current authority | ContractSlice A is merged/exact-main green. B surface, codec, three wires, B-RESOURCE, and B-CACHE are exact-head hosted green; R=0006 and cache R=0007 are accepted/immutable. B is `8/11`, A+B `40/43`; admission R=0008/R=0009/R=0010 are immutable diagnostics/no-success, and B-ADMISSION is third-corrected `FrozenDesign`/inactive pending this exact correction-design head's hosted gate. Later B, C/D, activation, merge/release/publication remain held. |
