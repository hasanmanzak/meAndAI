...
| Status | Accepted architecture; [SUBF-0143](../../features/FEAT-0065-shared-executable-conformance-runtime/README.md#subf-0143) Gate 2 accepted. ContractSlice A is merged/exact-main green; B surface, codec, three wire packets, B-RESOURCE, and B-CACHE are exact-head hosted green. Resource R=0006 and cache R=0007 are accepted/immutable. B is `8/11`, cumulative A+B `40/43`; admission R=0008 is immutable diagnostic/no-success, and B-ADMISSION is corrected `FrozenDesign`/inactive pending this exact corrected-design head's hosted gate. Later packets and the parent scenario remain inactive; no activation/DoD is claimed. |
...
| Current slice boundary | ContractSlice A is immutable merged history. ContractSlice B surface, codec, three wires, B-RESOURCE, and B-CACHE are hosted green; cache R=0007 is accepted. B is `8/11`, A+B `40/43`; admission R=0008 is immutable diagnostic/no-success, and B-ADMISSION is corrected `FrozenDesign`/inactive pending the corrected-design-head hosted gate, while later B, C/D, and final activation remain held. |
