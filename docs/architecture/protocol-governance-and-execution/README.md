...
| Status | Accepted architecture; [SUBF-0143](../../features/FEAT-0065-shared-executable-conformance-runtime/README.md#subf-0143) Gate 2 accepted. ContractSlice A is merged/exact-main green; B surface, codec, three wire packets, and B-RESOURCE are exact-head hosted green. Resource R=0006 and cache R=0007 are accepted/immutable; B-CACHE is `ReviewedLocalGreen`. B is `8/11`, cumulative A+B `40/43`; B-ADMISSION is candidate/inactive pending the cache implementation-head hosted gate and its own design freeze. Later packets and the parent scenario remain inactive; no activation/DoD is claimed. |
...
| Current slice boundary | ContractSlice A is immutable merged history. ContractSlice B surface, codec, three wires, and B-RESOURCE are hosted green; cache R=0007 is accepted and B-CACHE is `ReviewedLocalGreen`. B is `8/11`, A+B `40/43`; B-ADMISSION/later B, C/D, and final activation remain held. |
