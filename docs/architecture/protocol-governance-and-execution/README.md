...
| Status | Accepted architecture; [SUBF-0143](../../features/FEAT-0065-shared-executable-conformance-runtime/README.md#subf-0143) Gate 2 accepted. ContractSlice A, B, and C are merged/exact-main green; C is `11/11`. Policy activation, producer infrastructure, and the first-rules cohort are `ExactHeadHostedGreen`; RULE-0003 and RULE-0004 are `ReviewedLocalGreen`; D is `8/11` and cumulative A+B+C+D/full Conformance is `62/62`. The specialized-rules cohort is local `2/3` with push held; RULE-0005 is next/inactive. The parent scenario remains inactive and no final D activation or DoD is claimed. |
...
| Current slice boundary | ContractSlice A, B, and C are immutable merged/exact-main-green history. Policy activation, producer infrastructure, and the first-rules cohort are `ExactHeadHostedGreen`; `D-RULE-0003-01` and `D-RULE-0004-01` are `ReviewedLocalGreen`; D is `8/11` and A+B+C+D/full Conformance is `62/62`. The specialized-rules cohort is local `2/3` with push held; RULE-0005 is next/inactive and final activation remains held. |
