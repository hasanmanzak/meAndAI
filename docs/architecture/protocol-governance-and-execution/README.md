...
| Current slice boundary | ContractSlice A and B are immutable merged/exact-main-green history; B is `11/11`, A+B `43/43`. The C Activation cohort is `ExactHeadHostedGreen`; C is `5/11`, current A+B+C is `48/48`, and Applicability is the only next C cohort. ContractSlice D and final activation remain held. |
