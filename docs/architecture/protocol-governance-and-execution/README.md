...
| Status | Accepted architecture; [SUBF-0143](../../features/FEAT-0065-shared-executable-conformance-runtime/README.md#subf-0143) Gate 2 accepted. ContractSlice A and B are merged/exact-main green; B is `11/11`, cumulative A+B `43/43`. C Activation and Applicability are `ExactHeadHostedGreen`; Evaluation Plan is `ReviewedLocalGreen`/unpushed, C is `8/11`, current A+B+C `51/51`. R=0012/R=0013 are diagnostics/no-success; R=0014 is accepted/immutable. Evaluation Advance is next/`FrozenDesign`; the parent scenario remains inactive and no activation/DoD is claimed. |
...
| Current slice boundary | ContractSlice A and B are immutable merged/exact-main-green history; B is `11/11`, A+B `43/43`. C Activation and Applicability are `ExactHeadHostedGreen`; Evaluation Plan is `ReviewedLocalGreen`/unpushed, C is `8/11`, current A+B+C `51/51`; R=0014 is accepted/immutable. Evaluation Advance is next/`FrozenDesign`; ContractSlice D and final activation remain held. |
