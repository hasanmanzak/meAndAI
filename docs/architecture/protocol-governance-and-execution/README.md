...
| Status | Accepted architecture; [SUBF-0143](../../features/FEAT-0065-shared-executable-conformance-runtime/README.md#subf-0143) Gate 2 accepted. ContractSlice A and B are merged/exact-main green; B is `11/11`, cumulative A+B `43/43`. C surface and registration-mismatch are `ReviewedLocalGreen`; C is `4/11` and current A+B+C is `47/47`. `C-PRODUCER-PIPELINE-01` is next; the cohort remains unpushed. The parent scenario remains inactive; no activation/DoD is claimed. |
...
| Current slice boundary | ContractSlice A and B are immutable merged/exact-main-green history; B is `11/11`, A+B `43/43`. The C surface and registration-mismatch packets are `ReviewedLocalGreen`; C is `4/11`, current A+B+C is `47/47`, and producer-pipeline is next. ContractSlice D and final activation remain held. |
