...
| Current authority | Historical A-D and final-activation records own the closed predecessor. The current directive authorizes only [SUBF-0144](../../features/FEAT-0065-shared-executable-conformance-runtime/README.md#subf-0144) design until `AcceptedFrozenDesign`; its implementation activates automatically only after the exact design head is hosted green. Feature DoD, WIP extraction, consumer mutation, merge, release, publication, authority transfer, and PowerShell retirement remain held. |
