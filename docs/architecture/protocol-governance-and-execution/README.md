...
| Current slice boundary | ContractSlice A is immutable merged history. ContractSlice B structural surface, codec activation, and repository-tree wire are hosted green; governed-text is reviewed-local-green at B `5/11`, A+B `37/43`, while its implementation-head hosted gate, later B packets, C/D, and final activation remain held. |
