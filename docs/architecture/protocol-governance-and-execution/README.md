...
| Current slice boundary | ContractSlice A is immutable merged history. ContractSlice B surface, codec, three wires, B-RESOURCE, and B-CACHE are hosted green; cache R=0007 is accepted. B is `8/11`, A+B `40/43`; B-ADMISSION is `FrozenDesign`/inactive pending the design-head hosted gate, while later B, C/D, and final activation remain held. |
