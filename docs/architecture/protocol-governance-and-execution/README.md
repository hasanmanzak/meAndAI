...
| Current slice boundary | ContractSlice A is immutable merged history. ContractSlice B surface, codec, three wires, B-RESOURCE, and B-CACHE are hosted green; cache R=0007 is accepted. B is `8/11`, A+B `40/43`; admission R=0008/R=0009 are immutable diagnostics/no-success, and B-ADMISSION is second-corrected `FrozenDesign`/inactive pending the correction-design-head hosted gate, while later B, C/D, and final activation remain held. |
