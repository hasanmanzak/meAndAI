...
| Status | Accepted architecture; [SUBF-0143](../../features/FEAT-0065-shared-executable-conformance-runtime/README.md#subf-0143) Gate 2 accepted. ContractSlice A and B are merged/exact-main green; B is `11/11`, cumulative A+B `43/43`. All three C Activation-cohort packets are `ReviewedLocalGreen`; C is `5/11` and current A+B+C is `48/48`. The cohort is locally complete but remains unpushed/hosted-pending. The parent scenario remains inactive; no activation/DoD is claimed. |
...
| Current slice boundary | ContractSlice A and B are immutable merged/exact-main-green history; B is `11/11`, A+B `43/43`. The three C Activation packets are `ReviewedLocalGreen`; C is `5/11`, current A+B+C is `48/48`, and the cohort awaits one push plus exact-head hosted validation. ContractSlice D and final activation remain held. |
