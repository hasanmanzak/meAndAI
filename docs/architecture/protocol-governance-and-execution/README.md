...
| Status | Accepted architecture; [SUBF-0143](../../features/FEAT-0065-shared-executable-conformance-runtime/README.md#subf-0143) Gate 2 accepted. ContractSlice A and B are merged/exact-main green; B is `11/11`, cumulative A+B `43/43`. The C Activation cohort is `ExactHeadHostedGreen`; both Applicability packets are `ReviewedLocalGreen`, C is `7/11`, and current A+B+C is `50/50`. R=0007/R=0011 are accepted/immutable; Applicability is locally complete with one push/exact-head hosted gate pending and Evaluation held. The parent scenario remains inactive; no activation/DoD is claimed. |
...
| Current slice boundary | ContractSlice A and B are immutable merged/exact-main-green history; B is `11/11`, A+B `43/43`. The C Activation cohort is `ExactHeadHostedGreen`; both Applicability packets are `ReviewedLocalGreen`, C is `7/11`, current A+B+C is `50/50`, and the cohort awaits its single push/exact-head hosted gate before Evaluation. ContractSlice D and final activation remain held. |
