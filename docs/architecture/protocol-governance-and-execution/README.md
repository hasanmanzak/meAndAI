...
| Current slice boundary | ContractSlice A is immutable merged history. ContractSlice B structural surface, codec activation, repository-tree, and governed-text are hosted green at B `5/11`, A+B `37/43`; repository-target is frozen/inactive pending its exact design-head hosted gate, while later B packets, C/D, and final activation remain held. |
