...
| Status | Accepted architecture; [SUBF-0143](../../features/FEAT-0065-shared-executable-conformance-runtime/README.md#subf-0143) Gate 2 accepted. ContractSlice A and B are merged/exact-main green; B is `11/11`, cumulative A+B `43/43`. `C-SURFACE-ACTIVATION-01` is `ReviewedLocalGreen`; C is `3/11` and current A+B+C is `46/46`. `C-REGISTRATION-MISMATCH-01` is next; the cohort remains unpushed. The parent scenario remains inactive; no activation/DoD is claimed. |
...
| Current slice boundary | ContractSlice A and B are immutable merged/exact-main-green history; B is `11/11`, A+B `43/43`. The C surface packet is `ReviewedLocalGreen`; C is `3/11`, current A+B+C is `46/46`, and registration-mismatch is next. ContractSlice D and final activation remain held. |
