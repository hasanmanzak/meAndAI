...
| Transition state | Accepted architecture; A is immutable merged history; B surface, codec, three wires, B-RESOURCE, and B-CACHE are hosted green. Cache R=0007 is accepted; admission R=0008 is immutable diagnostic/no-success, and B-ADMISSION is corrected `FrozenDesign`/inactive pending this exact synchronized corrected-design head's hosted gate. Later B, C/D, final activation, WIP, completion/DoD remain held. |
