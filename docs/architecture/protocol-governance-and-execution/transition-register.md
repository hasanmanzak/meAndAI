...
| Current slice boundary | ContractSlice A and B are merged/exact-main green, with B `11/11` and A+B `43/43`. All three C Activation-cohort packets are `ReviewedLocalGreen`; C is `5/11` and current A+B+C is `48/48`. The locally complete cohort has not been pushed and has no hosted-green claim. The parent scenario remains held. |
...
| Transition state | Accepted architecture; A and B are immutable merged/exact-main-green history. C is the sole active slice: its three Activation packets are `ReviewedLocalGreen`, and the locally complete cohort remains unpushed/hosted-pending. D, final activation, WIP, completion/DoD remain held. |
