...
| Transition state | Accepted architecture; A is immutable merged history; B structural surface, codec activation, repository-tree, governed-text, and repository-target are hosted green; repository-target R=0004 remains diagnostic-only and canonical R=0005 remains accepted. `B-RESOURCE-01` is `FrozenDesign`/inactive pending its synchronized design-head hosted gate; later B runtime packets, C/D, final activation, WIP, completion/DoD remain held. |
