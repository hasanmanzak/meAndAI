...
| Current slice boundary | ContractSlice A and B are merged/exact-main green, with B `11/11` and A+B `43/43`. `C-SURFACE-ACTIVATION-01` is `ReviewedLocalGreen`; C is `3/11` and current A+B+C is `46/46`. `C-REGISTRATION-MISMATCH-01` is next; the Activation cohort has not been pushed and has no hosted-green claim. The parent scenario remains held. |
...
| Transition state | Accepted architecture; A and B are immutable merged/exact-main-green history. C is the sole active slice: its surface packet is `ReviewedLocalGreen`, registration-mismatch is next, and the Activation cohort remains unpushed. D, final activation, WIP, completion/DoD remain held. |
