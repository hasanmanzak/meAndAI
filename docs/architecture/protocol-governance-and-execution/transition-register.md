...
| Current slice boundary | ContractSlice A is merged/exact-main green; B surface, codec activation, and repository-tree wire are exact-head hosted green; governed-text is `ReviewedLocalGreen` with its implementation-head hosted gate pending at B `5/11`, A+B `37/43`; parent scenario held. |
...
| Transition state | Accepted architecture; A is immutable merged history; B structural surface, codec activation, and repository-tree wire are hosted green, governed-text is reviewed-local-green, and its implementation-head hosted gate plus later B runtime packets, C/D, final activation, WIP, completion/DoD remain held. |
