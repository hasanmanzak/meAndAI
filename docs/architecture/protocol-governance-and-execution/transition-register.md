...
| Current slice boundary | ContractSlice A and B are merged/exact-main green, with B `11/11` and A+B `43/43`. C surface and registration-mismatch are `ReviewedLocalGreen`; C is `4/11` and current A+B+C is `47/47`. `C-PRODUCER-PIPELINE-01` is next; the Activation cohort has not been pushed and has no hosted-green claim. The parent scenario remains held. |
...
| Transition state | Accepted architecture; A and B are immutable merged/exact-main-green history. C is the sole active slice: its surface and registration-mismatch packets are `ReviewedLocalGreen`, producer-pipeline is next, and the Activation cohort remains unpushed. D, final activation, WIP, completion/DoD remain held. |
