...
| Current slice boundary | ContractSlice A, B, and C are merged/exact-main green; C is `11/11`. Policy activation is `ExactHeadHostedGreen`; producer infrastructure is `ReviewedLocalGreen`: D is `4/11` and A+B+C+D/full Conformance is `58/58`. Cohort 2 exact-head hosted validation and the parent scenario remain held. |
...
| Transition state | Accepted architecture; A, B, and C are immutable merged/exact-main-green history. Policy activation is `ExactHeadHostedGreen`; real producer infrastructure is `ReviewedLocalGreen` pending Cohort 2 exact-head hosted validation. RULE-0001, final activation, WIP, completion/DoD remain held. |
