...
| Current slice boundary | ContractSlice A is merged/exact-main green; B surface, codec, three wires, and B-RESOURCE are exact-head hosted green. Resource R=0006 and cache R=0007 are accepted/immutable; B-CACHE is `ReviewedLocalGreen`. B is `8/11`, A+B `40/43`; B-ADMISSION and the parent scenario remain held. |
...
| Transition state | Accepted architecture; A is immutable merged history; B surface, codec, three wires, and B-RESOURCE are hosted green. Cache R=0007 is accepted and B-CACHE is `ReviewedLocalGreen`; B-ADMISSION is candidate/inactive pending cache implementation-head hosted green and its own design freeze. Later B, C/D, final activation, WIP, completion/DoD remain held. |
