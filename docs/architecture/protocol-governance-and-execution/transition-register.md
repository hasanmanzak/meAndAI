...
| Current slice boundary | ContractSlice A is merged/exact-main green; B surface, codec activation, and repository-tree wire are exact-head hosted green; governed-text is `FrozenDesign`/inactive pending its synchronized design head's hosted gate at B `4/11`, A+B `36/43`; parent scenario held. |
...
| Transition state | Accepted architecture; A is immutable merged history; B structural surface, codec activation, and repository-tree wire are hosted green, governed-text is design-frozen/inactive, and its hosted design gate plus later B runtime packets, C/D, final activation, WIP, completion/DoD remain held. |
