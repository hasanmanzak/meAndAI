...
| Transition state | Accepted architecture; A is immutable merged history; B structural surface, codec activation, repository-tree, and governed-text are hosted green; repository-target R=0004 remains diagnostic-only, corrected R=0005 is frozen/inactive, and its exact correction-design-head hosted gate plus later B runtime packets, C/D, final activation, WIP, completion/DoD remain held. |
