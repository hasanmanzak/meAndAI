...
| Current slice boundary | ContractSlice A is merged/exact-main green; B surface, codec, three wires, and B-RESOURCE are exact-head hosted green. Resource R=0006 is accepted/immutable. B is `7/11`, A+B `39/43`; B-CACHE is `FrozenDesign`/inactive pending this exact design head's hosted gate. The parent scenario remains held. |
...
| Transition state | Accepted architecture; A is immutable merged history; B surface, codec, three wires, and B-RESOURCE are hosted green. B-CACHE is `FrozenDesign`/inactive pending this exact design head's hosted gate; later B, C/D, final activation, WIP, completion/DoD remain held. |
