...
| Transition state | Accepted architecture; A is immutable merged history; B surface, codec, three wires, B-RESOURCE, and B-CACHE are hosted green. Cache R=0007 is accepted; B-ADMISSION is `FrozenDesign`/inactive pending this exact synchronized design head's hosted gate. Later B, C/D, final activation, WIP, completion/DoD remain held. |
