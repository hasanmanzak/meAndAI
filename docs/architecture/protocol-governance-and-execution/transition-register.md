...
| Current slice boundary | ContractSlice A and B are merged/exact-main green, with B `11/11` and A+B `43/43`. C Activation and Applicability are `ExactHeadHostedGreen`; C is `7/11` and current A+B+C is `50/50`. R=0012/R=0013 are diagnostics/no-success and corrected R=0014 is inactive pending correction-head hosted green; the parent scenario remains held. |
...
| Transition state | Accepted architecture; A and B are immutable merged/exact-main-green history. C is the sole active slice: Activation and Applicability are exact-head hosted green, R=0012/R=0013 are diagnostics/no-success, and corrected R=0014 is frozen/inactive pending its correction delivery gate. D, final activation, WIP, completion/DoD remain held. |
