...
| Current slice boundary | ContractSlice A and B are merged/exact-main green, with B `11/11` and A+B `43/43`. C Activation, Applicability, and Evaluation are `ExactHeadHostedGreen`; C is `10/11` and current A+B+C is `53/53`. R=0016 is diagnostic/no-success and R=0017 accepted/immutable; `C-INTENT-RESULT-01` is `ReviewedLocalGreen`, `C-AGGREGATION-01` remains next/`FrozenDesign`, and the parent scenario remains held. |
...
| Transition state | Accepted architecture; A and B are immutable merged/exact-main-green history. C is the sole active slice: Activation, Applicability, and Evaluation are exact-head hosted green; R=0012/R=0013/R=0016 are diagnostics/no-success and R=0014/R=0015/R=0017 are accepted/immutable. `C-INTENT-RESULT-01` is `ReviewedLocalGreen`; `C-AGGREGATION-01` is the next frozen packet. D, final activation, WIP, completion/DoD remain held. |
