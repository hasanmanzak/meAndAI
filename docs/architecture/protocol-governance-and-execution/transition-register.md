...
| Current slice boundary | ContractSlice A and B are merged/exact-main green, with B `11/11` and A+B `43/43`. C Activation and Applicability are `ExactHeadHostedGreen`; Evaluation Plan is `ReviewedLocalGreen`/unpushed, C is `8/11` and current A+B+C is `51/51`. R=0012/R=0013 are diagnostics/no-success and R=0014 is accepted/immutable; Evaluation Advance is next/`FrozenDesign`, and the parent scenario remains held. |
...
| Transition state | Accepted architecture; A and B are immutable merged/exact-main-green history. C is the sole active slice: Activation and Applicability are exact-head hosted green, Evaluation Plan is packet-local `ReviewedLocalGreen`, R=0012/R=0013 are diagnostics/no-success, R=0014 is accepted/immutable, and Evaluation Advance is next/`FrozenDesign`. D, final activation, WIP, completion/DoD remain held. |
