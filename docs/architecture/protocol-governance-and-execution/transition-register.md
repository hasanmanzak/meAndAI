...
| Transition state | Accepted architecture; A is immutable merged history; B surface, codec, three wires, B-RESOURCE, B-CACHE, B-ADMISSION, and B-SEALED-CONTEXT are hosted green. Cache R=0007, admission R=0011, sealed-context R=0012, and corrected codec-derivation R=0014 are accepted; admission R=0008/R=0009/R=0010 plus codec-derivation R=0013 remain immutable diagnostics/no-success. Corrected R=0014 is local green pending its exact implementation head's hosted gate. B-CONVERGE, C/D, final activation, WIP, completion/DoD remain held. |
