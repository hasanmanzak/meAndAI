...
| Current slice boundary | ContractSlice A and B are merged/exact-main green, with B `11/11` and A+B `43/43`. ContractSlice C is `FrozenDesign`/inactive at `0/11`, pending its exact design-head hosted gate. The parent scenario remains held. |
...
| Transition state | Accepted architecture; A and B are immutable merged/exact-main-green history. C is the sole next slice and remains `FrozenDesign`/inactive until the synchronized design head passes hosted validation. D, final activation, WIP, completion/DoD remain held. |
