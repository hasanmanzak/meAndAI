...
| Current slice boundary | ContractSlice A and B are merged/exact-main green, with B `11/11` and A+B `43/43`. The C Activation cohort is `ExactHeadHostedGreen`; C is `5/11` and current A+B+C is `48/48`. Applicability is the sole next C cohort; the parent scenario remains held. |
...
| Transition state | Accepted architecture; A and B are immutable merged/exact-main-green history. C is the sole active slice: Activation is exact-head hosted green; applicability R=0006 is diagnostic/no-success and corrected R=0007 is frozen/inactive pending corrected-design hosted green. D, final activation, WIP, completion/DoD remain held. |
