...
| Current slice boundary | ContractSlice A is merged/exact-main green; B surface, codec, three wires, B-RESOURCE, B-CACHE, and B-ADMISSION are exact-head hosted green. Resource R=0006, cache R=0007, admission R=0011, and sealed-context R=0012 are accepted/immutable. B-SEALED-CONTEXT is local green with its implementation head hosted pending; B is `10/11`, A+B `42/43`. Admission R=0008/R=0009/R=0010 are immutable diagnostics/no-success; B-CODEC-DERIVATION is `FrozenDesign`/inactive pending that synchronized head's hosted gate; B-CONVERGE and the parent scenario remain held. |
...
| Transition state | Accepted architecture; A is immutable merged history; B surface, codec, three wires, B-RESOURCE, B-CACHE, and B-ADMISSION are hosted green. Cache R=0007, admission R=0011, and sealed-context R=0012 are accepted; admission R=0008/R=0009/R=0010 are immutable diagnostics/no-success. B-SEALED-CONTEXT is `ReviewedLocalGreen` with its implementation head hosted pending, while B-CODEC-DERIVATION is `FrozenDesign`/inactive pending that synchronized head's hosted gate. B-CONVERGE, C/D, final activation, WIP, completion/DoD remain held. |
