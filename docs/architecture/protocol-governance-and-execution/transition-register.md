...
| Current slice boundary | ContractSlice A is merged/exact-main green; B surface, codec, and three wires are exact-head hosted green. B-RESOURCE R=0006 is accepted and local green at B `7/11`, A+B `39/43`, with exact implementation-head hosted pending; B-CACHE and the parent scenario remain held. |
...
| Transition state | Accepted architecture; A is immutable merged history; B surface, codec, and three wires are hosted green. B-RESOURCE R=0006 is accepted and local green with exact implementation-head hosted pending; B-CACHE/later B, C/D, final activation, WIP, completion/DoD remain held. |
