...
| Current slice boundary | ContractSlice A, B, and C are merged/exact-main green; C is `11/11`. Policy activation is `ReviewedLocalGreen`: D is `3/11`, A+B+C+D/full Conformance is `57/57`, and the singleton cohort exact-head hosted gate is pending; the parent scenario remains held. |
...
| Transition state | Accepted architecture; A, B, and C are immutable merged/exact-main-green history. The frozen Policy-activation packet is `ReviewedLocalGreen`; its singleton cohort hosted gate and all later D work remain held. Final activation, WIP, completion/DoD remain held. |
