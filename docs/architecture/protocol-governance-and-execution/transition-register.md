...
| Transition state | Accepted architecture; A and B are immutable merged/exact-main-green history. C is the sole active slice: Activation is exact-head hosted green and `C-APPLICABILITY-PLAN-01` is `FrozenDesign`/inactive pending its records/design exact-head hosted gate. D, final activation, WIP, completion/DoD remain held. |
