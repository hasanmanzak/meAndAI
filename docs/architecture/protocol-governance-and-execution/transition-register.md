...
| Transition state | Accepted architecture; A is immutable merged history; B surface, codec, three wires, B-RESOURCE, B-CACHE, and B-ADMISSION are hosted green. Cache R=0007 and admission R=0011 are accepted; admission R=0008/R=0009/R=0010 are immutable diagnostics/no-success, and B-SEALED-CONTEXT is `FrozenDesign`/inactive pending its synchronized design-head hosted gate. Later B, C/D, final activation, WIP, completion/DoD remain held. |
