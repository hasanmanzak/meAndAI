...
| Current slice boundary | ContractSlice A is merged/exact-main green; B surface, codec activation, repository-tree, and governed-text are exact-head hosted green at B `5/11`, A+B `37/43`; repository-target is `FrozenDesign`/inactive pending its exact design-head hosted gate; parent scenario held. |
...
| Transition state | Accepted architecture; A is immutable merged history; B structural surface, codec activation, repository-tree, and governed-text are hosted green; repository-target is frozen/inactive, and its exact design-head hosted gate plus later B runtime packets, C/D, final activation, WIP, completion/DoD remain held. |
