...
| Current slice boundary | ContractSlice A, B, and C are merged/exact-main green; C is `11/11`. Policy activation, producer infrastructure, and the first-rules cohort are `ExactHeadHostedGreen`; RULE-0003 through RULE-0005 are `ReviewedLocalGreen`: D is `9/11` and A+B+C+D/full Conformance is `63/63`. The specialized-rules cohort is local `3/3` with its single push pending; equivalence/closure remains inactive and the parent scenario remains held. |
...
| Transition state | Accepted architecture; A, B, and C are immutable merged/exact-main-green history. Policy activation, real producer infrastructure, and the first-rules cohort are `ExactHeadHostedGreen`; RULE-0003 through RULE-0005 are `ReviewedLocalGreen`. The specialized-rules cohort is local `3/3` with its single push pending; equivalence/closure remains inactive; final activation, WIP, completion/DoD remain held. |
