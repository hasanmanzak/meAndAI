...
| Transition state | Accepted architecture; A, B, and C are immutable merged/exact-main-green history. Policy activation is `ExactHeadHostedGreen`; real producer infrastructure alone is frozen/inactive pending its design-head hosted gate. Final activation, WIP, completion/DoD remain held. |
