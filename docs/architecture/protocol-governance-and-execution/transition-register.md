...
| Transition state | Accepted architecture; A is immutable merged history; B surface, codec, three wires, B-RESOURCE, and B-CACHE are hosted green. Cache R=0007 and admission R=0011 are accepted; admission R=0008/R=0009/R=0010 are immutable diagnostics/no-success, and B-ADMISSION is `ReviewedLocalGreen` with its implementation head hosted pending. Later B, C/D, final activation, WIP, completion/DoD remain held. |
