...
| Current slice boundary | ContractSlice A and B are merged/exact-main green, with B `11/11` and A+B `43/43`. The C Activation cohort is `ExactHeadHostedGreen`; applicability plan is `ReviewedLocalGreen`, C is `6/11`, and current A+B+C is `49/49`. Applicability closure is the sole next packet; the parent scenario remains held. |
...
| Transition state | Accepted architecture; A and B are immutable merged/exact-main-green history. C is the sole active slice: Activation is exact-head hosted green; applicability plan is `ReviewedLocalGreen`, R=0007 is accepted/immutable, and closure is next/`FrozenDesign` without a cohort push/hosted claim. D, final activation, WIP, completion/DoD remain held. |
