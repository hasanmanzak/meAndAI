...
| Current slice boundary | ContractSlice A is merged/exact-main green; B surface, codec activation, repository-tree, and governed-text are exact-head hosted green. Repository-target R=0004 is diagnostic-only; canonical R=0005 is accepted/local green at B `6/11`, A+B `38/43`, with its implementation head hosted pending. `B-RESOURCE-01` and the parent scenario remain held. |
...
| Transition state | Accepted architecture; A is immutable merged history; B structural surface, codec activation, repository-tree, and governed-text are hosted green; repository-target R=0004 remains diagnostic-only and canonical R=0005 is accepted/local green. Its implementation-head hosted gate, `B-RESOURCE-01`, later B runtime packets, C/D, final activation, WIP, completion/DoD remain held. |
