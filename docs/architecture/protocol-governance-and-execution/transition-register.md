...
| Current slice boundary | ContractSlice A and B are merged/exact-main green, with B `11/11` and A+B `43/43`. The C Activation cohort is `ExactHeadHostedGreen`; both Applicability packets are `ReviewedLocalGreen`, C is `7/11`, and current A+B+C is `50/50`. The cohort awaits its single push/exact-head hosted gate; Evaluation and the parent scenario remain held. |
