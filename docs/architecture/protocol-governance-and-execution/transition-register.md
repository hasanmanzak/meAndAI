...
| Current slice boundary | ContractSlice A, B, and C are merged/exact-main green; C is `11/11` and A+B+C/full Conformance is `54/54`. ContractSlice D is a records-only frozen design candidate pending exact-head hosted delivery and explicit maintainer acceptance; the parent scenario remains held. |
...
| Transition state | Accepted architecture; A, B, and C are immutable merged/exact-main-green history. D is design-only/inactive; exact-head hosted design delivery and explicit maintainer acceptance are required before implementation. Final activation, WIP, completion/DoD remain held. |
