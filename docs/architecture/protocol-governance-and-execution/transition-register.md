...
| Current slice boundary | ContractSlice A, B, and C are merged/exact-main green; C is `11/11`. Policy activation and producer infrastructure are `ExactHeadHostedGreen`; RULE-0001 and RULE-0002 are `ReviewedLocalGreen`: D is `6/11` and A+B+C+D/full Conformance is `60/60`. The first-rules cohort is local green with push/hosted validation pending; RULE-0003 is next/inactive and the parent scenario remains held. |
...
| Transition state | Accepted architecture; A, B, and C are immutable merged/exact-main-green history. Policy activation and real producer infrastructure are `ExactHeadHostedGreen`; RULE-0001 and RULE-0002 are `ReviewedLocalGreen`. The first-rules cohort is local green with push/hosted validation pending; RULE-0003 is next/inactive; final activation, WIP, completion/DoD remain held. |
