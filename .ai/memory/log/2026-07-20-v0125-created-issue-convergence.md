...
- Its first Derdini replay repaired the exact quote-stripped issue #7 and
  created canonical v0.12.4 issue #8, but then stopped before branch or PR
  publication with `The canonical protocol-update issue did not converge after
  creation.`
- A fresh GitHub read proved issue #8 had the exact title, marker, body, state,
...
- `BUG-0021` / `FIND-0197` captures the false convergence failure.
...
- Created-issue true zero-list lag plus different-identity race focus passed as
  `TEST-0150` in 12.0 seconds, including separate updater and issue-token actors.
- `TEST-0148` and `TEST-0149` passed in 23.5 seconds; `TEST-0147` passed in
...
against Derdini, then reconcile PR #6 and stale issues #7/#8 only after the new
