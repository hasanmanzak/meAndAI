...
- `SUBF-0078` and `SUBF-0079` are locally complete. Hosted delivery,
  publication, and release closure remain pending external facts.

## Focused and supporting evidence

- TEST-0161 passed with exact operation observations `2/2` process starts and
  `4/4` requests: PowerShell 7 in 84.1 seconds and Windows PowerShell 5.1 in
  147.6 seconds. TEST-0151 and TEST-0152 remained green in the same owner.
- TEST-0162 passed under PowerShell 7 in 5.3 seconds and Windows PowerShell 5.1
  in 6.5 seconds. TEST-0158 and TEST-0159 remained green in the same owner.
...
- No wall-clock improvement is claimed. The local PS5.1 TEST-0161 observation,
...
- The bounded convergence scan found two Blocking observations. `FIND-0200`
  added missing TEST-0162 ownership for the third independent expected reader;
  PS5.1/PS7 focused confirmations passed in 9.3/9.2 seconds and StructureOnly
  passed in 7.8 seconds. `FIND-0201` reconciled actual local gate evidence with
  the feature/index/DoD/memory projection. No unresolved `Blocking` finding
  remains. `FIND-0202`/`FIND-0203` are optional bounded improvements, while
  `FIND-0204` keeps the unimproved Windows critical path owned by issue #98.
...
assets, cleanup, and post-publication evidence through issue #98. None of those
