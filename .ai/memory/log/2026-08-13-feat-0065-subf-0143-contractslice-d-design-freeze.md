...
- `D-REAL-PRODUCER-INFRASTRUCTURE-01`, the first-rules cohort, and the specialized-rules cohort are immutable `ExactHeadHostedGreen`. Repository/provider equivalence is `ReviewedLocalGreen`; activation topology is the sole active packet. The parent scenario remains Planned; final activation, runtime-efficiency scenario, merge, release, publication, and DoD remain held.
