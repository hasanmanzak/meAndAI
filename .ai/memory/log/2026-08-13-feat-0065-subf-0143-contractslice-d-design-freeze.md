...
- D predecessor cohorts are immutable `ExactHeadHostedGreen`. Repository/provider equivalence and activation topology are `ReviewedLocalGreen`; code-free D-CONVERGE is the sole active packet. The parent scenario remains Planned; final activation, runtime-efficiency scenario, merge, release, publication, and DoD remain held.
