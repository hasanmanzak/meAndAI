...
- Historical ContractSlice A strict-redraw and parser-record-slot chain: base
  ...
  correctly skipped. ContractSlice A is merged/exact-main green. B surface, codec, three wire packets, B-RESOURCE, and B-CACHE are exact-head hosted green. Repository-target R=0004 plus admission R=0008/R=0009 are immutable diagnostics/no-success; R=0005, resource R=0006, and cache R=0007 remain accepted/immutable. B is `8/11`, cumulative A+B is `40/43`; B-ADMISSION is second-corrected `FrozenDesign`/inactive pending this exact synchronized correction-design head's hosted gate. The parent scenario, later B packets, C/D, and final activation remain held. Follow the current [canonical log index](log/README.md). The
